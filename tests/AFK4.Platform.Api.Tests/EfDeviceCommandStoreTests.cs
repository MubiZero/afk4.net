...
public sealed class EfDeviceCommandStoreTests
{
    [Fact]
    public async Task CommandStatus_PersistsAcrossStoreInstances()
    {
        ...
        await using (var db = new PlatformDbContext(options))
        {
            ...
            Assert.Equal(DateTimeOffset.Parse("2026-05-12T00:00:00Z"), status.CreatedAtUtc);
