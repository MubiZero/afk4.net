...
public sealed class PlatformTenantHealthEndpointTests
{
    [Fact]
    public async Task GetHealth_WithValidTenant_ReturnsCountsAndStatus()
    {
        ...
        var response = await client.GetAsync($"/api/platform/tenants/{organizationId:D}/health");
        var body = await response.Content.ReadFromJsonAsync<TenantHealthDto>();
    ...
    [Fact]
    public async Task GetHealth_WithUnknownTenant_Returns404()
    {
        await using var factory = new PlatformApiFactory();
        using var client = factory.CreateClient();
        await PlatformAdminTestHelper.AuthorizeAsAsync(factory, client);

        var response = await client.GetAsync($"/api/platform/tenants/{Guid.NewGuid():D}/health");
    ...
    public async Task GetHealth_WithoutPermission_Returns403AndAuditsDenied()
    {
        ...
        var audit = await dbContext.AuditRecords.SingleAsync(record => record.Action == AuditActionNames.ViewTenantHealth);
    ...
    public async Task GetHealth_WritesSucceededAuditWithSummary()
    {
        ...
        var audit = await dbContext.AuditRecords
            .Where(record => record.Action == AuditActionNames.ViewTenantHealth && record.Outcome == AuditOutcome.Succeeded)
            .SingleAsync();
