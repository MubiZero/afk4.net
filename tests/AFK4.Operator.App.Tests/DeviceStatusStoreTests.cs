...
public sealed class DeviceStatusStoreTests
{
    ...
    [Fact]
    public void Apply_ReturnsFalseForUnknownMachine()
    {
        var viewModel = new MainWindowViewModel();
        var status = new DeviceStatusChangedDto(
            OrganizationId: Guid.Parse("0c04d6c0-bfa8-4e26-9263-fc0d307d0f08"),
    ...
    [Fact]
    public void Apply_MapsOnlineLockedDeviceToLocked()
    {
        var viewModel = new MainWindowViewModel();
        var status = new DeviceStatusChangedDto(
            OrganizationId: Guid.Parse("0c04d6c0-bfa8-4e26-9263-fc0d307d0f08"),
        ...
        Assert.True(updated);
        Assert.Contains(viewModel.Seats, seat => seat.Name == "PC-001" && seat.State == "Locked" && seat.IsOnline);
    }
}
