...
{
    // Это и есть настоящий предел проверки: по нему работник останавливается, и после него ждать
    // кадр уже бессмысленно. Двадцати секунд не хватало там, где нужен второй круг цикла —
    // оформление приезжает сердцебиением, а публикуется на следующем заходе. На загруженном
    // раннере работник не доживал до него, а проверка потом ещё минуту ждала кадр, которого
    // никто не пришлёт. Запас бесплатный: удачный кадр отменяет ожидание сам.
