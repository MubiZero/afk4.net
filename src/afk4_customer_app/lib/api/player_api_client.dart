...
class PlayerApiClient {
  ...
  /// `no_seats_available` — свободных ПК в зале на это время не осталось,
  ...
  /// `no_seats_available` — столько свободных ПК на это время в зале нет.
  ///
  ...
  /// Выход: сервер гасит предъявленную пару токенов. Телефон бывает общим, и «вышел» должно
  /// значить «этой парой больше не войти», а не «стёр у себя». Отказ не поднимается наверх —
  /// локальный выход состоится в любом случае, иначе человек остался бы заперт в чужом аккаунте
  ...
  Future<void> signOut() async {
    ...
    try {
      await _send('POST', '/api/public/player/sign-out', body: {
        'refreshToken': current.refreshToken,
      });
    } on PlayerApiException {
      // Сеть легла — токены доживут свой срок, но с экрана человек уйдёт.
    }
  }
