...
describe('selectScreen — таблица экранов §3', () => {
  const cases: [string, Parameters<typeof selectScreen>[0], ShellScreen][] = [
    ...
    ['встал раньше — сначала итог', { state: state(PlayerShellStateNames.Locked), signedIn: true, approached: false, ended: true }, 'summary'],
    ['итог не для того, кто уже вышел', { state: state(PlayerShellStateNames.Locked), signedIn: false, approached: false, ended: true }, 'idle'],
    ['пока сессия не закрылась, итога нет', { state: state(PlayerShellStateNames.Active), signedIn: true, approached: false, ended: true }, 'session'],
