...
public sealed class OperatorWebAssetResolver
{
    ...
    public OperatorWebAssetResolver(string baseDirectory)
        : this(baseDirectory, File.Exists)
    {
    }

    public OperatorWebAssetResolver(string baseDirectory, Func<string, bool> fileExists)
    {
    ...
    public OperatorWebShellLaunchTarget Resolve(OperatorWebShellOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);

        if (options.DevServerUrl is not null)
        {
            return new OperatorWebShellLaunchTarget(options.DevServerUrl, "dev-server", LocalFolderPath: null);
        }
        ...
        throw new InvalidOperationException(
            "Operator frontend assets were not found. Build src/AFK4.Operator.App.Web or include WebAssets/index.html.");
    }
    ...
    private static OperatorWebShellLaunchTarget CreateLocalTarget(string indexPath, string mode)
    {
        ...
        return new OperatorWebShellLaunchTarget(
