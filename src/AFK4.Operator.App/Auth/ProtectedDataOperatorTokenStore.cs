...
public sealed class ProtectedDataOperatorTokenStore : IOperatorTokenStore
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    private readonly string path;

    public ProtectedDataOperatorTokenStore()
        : this(GetDefaultPath())
    {
    }

    public ProtectedDataOperatorTokenStore(string path)
    {
        this.path = path;
    }

    public async Task SaveAsync(OperatorTokenSnapshot snapshot, CancellationToken cancellationToken)
    {
    ...
    public async Task<OperatorTokenSnapshot?> LoadAsync(CancellationToken cancellationToken)
    {
        if (!File.Exists(path))
        {
            return null;
        ...
        try
        {
            ...
            return JsonSerializer.Deserialize<OperatorTokenSnapshot>(json, JsonOptions);
        }
    ...
    private static string GetDefaultPath()
    {
        ...
        return Path.Combine(localAppData, "AFK4", "Operator", "tokens.bin");
    }
}
