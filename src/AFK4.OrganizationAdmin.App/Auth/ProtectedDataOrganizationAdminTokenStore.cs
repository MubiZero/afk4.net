...
public sealed class ProtectedDataOrganizationAdminTokenStore : IOrganizationAdminTokenStore
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    private readonly string path;

    public ProtectedDataOrganizationAdminTokenStore()
        : this(GetDefaultPath())
    {
    }

    public ProtectedDataOrganizationAdminTokenStore(string path)
    {
        this.path = path;
    }

    public async Task SaveAsync(OrganizationAdminTokenSnapshot snapshot, CancellationToken cancellationToken)
    {
    ...
    public async Task<OrganizationAdminTokenSnapshot?> LoadAsync(CancellationToken cancellationToken)
    {
        if (!File.Exists(path))
        {
            return null;
        ...
        try
        {
            ...
            return JsonSerializer.Deserialize<OrganizationAdminTokenSnapshot>(json, JsonOptions);
        }
    ...
    private static string GetDefaultPath()
    {
        ...
        return Path.Combine(localAppData, "AFK4", "OrganizationAdmin", "tokens.bin");
    }
}
