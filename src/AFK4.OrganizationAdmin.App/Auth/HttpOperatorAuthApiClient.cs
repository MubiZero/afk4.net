...
public sealed class HttpOperatorAuthApiClient(HttpClient httpClient, IOrganizationAdminTokenStore tokenStore) : IOperatorAuthApiClient
{
    ...
    private Task SaveSnapshotAsync(StaffSignInResponse response, CancellationToken cancellationToken)
    {
        return tokenStore.SaveAsync(
            new OrganizationAdminTokenSnapshot(
