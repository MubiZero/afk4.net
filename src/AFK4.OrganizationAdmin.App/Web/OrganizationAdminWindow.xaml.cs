...
public partial class OrganizationAdminWindow : Window
{
    ...
    private readonly OrganizationAdminOptions appOptions;
    ...
    private OrganizationAdminWindow(OrganizationAdminOptions appOptions)
        : this(
            appOptions,
            OrganizationAdminWebShellOptions.LoadFromEnvironment(),
            new OrganizationAdminWebAssetResolver(AppContext.BaseDirectory),
    ...
    public OrganizationAdminWindow(
        ...
        OrganizationAdminWebShellOptions shellOptions,
        OrganizationAdminWebAssetResolver assetResolver)
        : this(
            appOptions,
            shellOptions,
            assetResolver,
    ...
    public OrganizationAdminWindow(
        ...
        OrganizationAdminWebShellOptions shellOptions,
        OrganizationAdminWebAssetResolver assetResolver,
        OrganizationAdminWebHostBridge hostBridge)
    {
    ...
    {
        var saved = OrganizationAdminWindowPlacementStore.Load();
    ...
    private void SaveCurrentPlacement()
    {
        ...
        OrganizationAdminWindowPlacementStore.Save(new OrganizationAdminWindowPlacement
        {
            Left = bounds.X,
            Top = bounds.Y,
            Width = bounds.Width,
    ...
    private async Task InitializeBrowserAsync()
    {
        try
        {
            ...
            var userDataFolder = OrganizationAdminWebView2UserDataFolder.EnsureExists();
            ...
            if (launchTarget.UsesLocalFolder)
            {
                Browser.CoreWebView2.SetVirtualHostNameToFolderMapping(
                    OrganizationAdminWebAssetResolver.LocalVirtualHost,
                    launchTarget.LocalFolderPath!,
                    CoreWebView2HostResourceAccessKind.Allow);
            }

            await Browser.CoreWebView2.AddScriptToExecuteOnDocumentCreatedAsync(
                OrganizationAdminWebBootstrapScript.Create(appOptions, launchTarget));
    ...
    // The dev harness runs the Organization Admin UI from a localhost vite dev server (env-provisioned).
    ...
    private void OnNavigationStarting(object? sender, CoreWebView2NavigationStartingEventArgs e)
    {
        ...
        if (string.Equals(target.Host, OrganizationAdminWebAssetResolver.LocalVirtualHost, StringComparison.OrdinalIgnoreCase))
        {
            return;
        }
    ...
    {
        // No popups / new windows from the Organization Admin UI.
        e.Handled = true;
    }
    ...
    private async void OnWebMessageReceived(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
    {
        ...
        {
            OrganizationAdminStartupLog.Write("Unhandled error in Organization Admin host bridge.", exception);
    ...
    private static OrganizationAdminWebHostBridge CreateDefaultHostBridge(OrganizationAdminOptions appOptions)
    {
        ArgumentNullException.ThrowIfNull(appOptions);
