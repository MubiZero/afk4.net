...
public sealed class ProtectedDataOrganizationAdminConnectionStore : IOrganizationAdminConnectionStore
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    private readonly string path;

    public ProtectedDataOrganizationAdminConnectionStore()
        : this(GetDefaultPath())
    {
    }

    public ProtectedDataOrganizationAdminConnectionStore(string path)
    {
        this.path = path;
    }

    public async Task SaveAsync(OrganizationAdminConnectionSnapshot snapshot, CancellationToken cancellationToken)
    {
    ...
    public async Task<OrganizationAdminConnectionSnapshot?> LoadAsync(CancellationToken cancellationToken)
    {
        if (!File.Exists(path))
        {
            return null;
        ...
        try
        {
            ...
            return JsonSerializer.Deserialize<OrganizationAdminConnectionSnapshot>(json, JsonOptions);
        }
    ...
    private static string GetDefaultPath()
    {
        ...
        return Path.Combine(localAppData, "AFK4", "OrganizationAdmin", "connection.bin");
    }
}
