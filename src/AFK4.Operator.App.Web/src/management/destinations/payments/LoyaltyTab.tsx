...
export function LoyaltyTab({ controller: c, currencyCode, hasBackend }: Props) {
  ...
  // Локальная карточка правила: замыкает t() из скоупа компонента, поэтому пропом не принимает
  // (иначе конкретный MessageKey-типизированный t не подходит под generic (string) => string).
  function RuleCard({
    enabled, onToggle, name, hint, percent, onPercent, percentAria, disabled
  }: {
