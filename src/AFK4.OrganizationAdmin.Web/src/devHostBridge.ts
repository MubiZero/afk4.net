// DEV-ONLY host-bridge stub. Emulates the native WebView2 host so the Organization Admin UI can run in a
...
//
// In the native WebView2 host the real bridge + injected __AFK4_ORGANIZATION_ADMIN_CONFIG__ already exist;
...
function installHostPreviewOverlay(native: DevWebView): void {
  ...
  const cfg = window.__AFK4_ORGANIZATION_ADMIN_CONFIG__;
  if (cfg) {
