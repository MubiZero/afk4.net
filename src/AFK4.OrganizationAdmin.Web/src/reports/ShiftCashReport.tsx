...
import { ReportRangeControls, ReportRangeSkeleton } from './ReportRangeControls';
...
export function ShiftCashReport({ backend, currencyCode }: { backend: OperatorBackendContext | null; currencyCode: string }): JSX.Element {
  ...
  return <ManagementScreen title={t('op.reports.shifts.title')} subtitle={t('op.reports.shifts.subtitle')} contentWidth="full" state={state} skeleton={<ShiftCashSkeleton />} failure={error} onRetry={() => void load()}>
