...
import type { CashOperationReportResultDto, ReportShiftRowDto, ShiftReportResultDto } from '../api/clients/reports';
...
export function ShiftCashReport({ backend, currencyCode }: { backend: OperatorBackendContext | null; currencyCode: string }): JSX.Element {
  ...
  const [data, setData] = useState<{ shifts: ShiftReportResultDto; cash: CashOperationReportResultDto } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [shifts, cash] = await Promise.all([reports.getShifts(backend.branchId, query), reports.getCashOperations(backend.branchId, query)]);
      setData({ shifts, cash });
      setSelectedId((current) => shifts.rows.some((row) => row.shiftId === current) ? current : shifts.rows[0]?.shiftId ?? null);
      setState('ready');
    } catch (reason) { setError(projectOperatorError(reason, t).detail); setState('error'); }
  }, [backend, range, t]);
  useEffect(() => { void load(); }, [load]);
  const selected: ReportShiftRowDto | undefined = data?.shifts.rows.find((row) => row.shiftId === selectedId);
  async function exportCsv() {
    if (!backend) return;
    const reports = createReportClients(backend);
    const [shifts, cash] = await Promise.all([reports.exportShifts(backend.branchId, toReportQuery(range)), reports.exportCashOperations(backend.branchId, toReportQuery(range))]);
    downloadTextFile(`shifts-cash-${range.from}-${range.to}.csv`, `${shifts}\r\n${cash}`);
  }
