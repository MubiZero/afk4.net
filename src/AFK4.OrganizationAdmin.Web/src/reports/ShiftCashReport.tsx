...
export function ShiftCashReport({ backend, currencyCode }: { backend: OperatorBackendContext | null; currencyCode: string }): JSX.Element {
  ...
  return <ManagementScreen title={t('op.reports.shifts.title')} subtitle={t('op.reports.shifts.subtitle')} contentWidth="full" state={state} failure={error} onRetry={() => void load()}>
