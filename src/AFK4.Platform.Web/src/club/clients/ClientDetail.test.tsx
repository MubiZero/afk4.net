...
it('shows the header and the edit-unavailable note', () => {
  render(
    <I18nProvider>
      <ClientDetail client={fakeClient() as never} packages={fakeClient() as never} player={player} branchId="b1" organizationId="org" canViewBilling />
    </I18nProvider>
  );
...
it('hides the wallet panel and shows a note when billing is not permitted', () => {
  ...
  render(
    <I18nProvider>
      <ClientDetail client={client as never} packages={fakeClient() as never} player={player} branchId="b1" organizationId="org" canViewBilling={false} />
    </I18nProvider>
  );
