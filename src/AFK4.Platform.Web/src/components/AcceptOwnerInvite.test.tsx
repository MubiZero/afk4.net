...
it('activates the owner and directs them to Operator App without exposing a staff session', async () => {
  const accept = mock(async () => {});
  render(<I18nProvider><AcceptOwnerInvite client={{ accept } as never} initialCode=" invite-1 " /></I18nProvider>);
  ...
  expect(screen.getByText('Теперь войдите с этой учётной записью в Operator App.')).toBeInTheDocument();
});
