    ...
    ILogger<EfPlatformPinService> logger) : IPlatformPinService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        //
        // Сетевого запрета среди причин нет намеренно. Дверь, отвечающая запрещённому иначе,
        // превращает экран игрового ПК в справочник «кто в этой сети под запретом» — то есть
        // заодно и «кто в этой сети есть». Запрет держится на действиях: войти запрещённый
        // войдёт и увидит свои деньги и причину, а начать на них новое не сможет ничем
        // (см. PlayerAuthenticationMiddleware).
        if (person is null
            || !person.IsActive
