...
builder.Services.AddScoped<PlatformAnnouncementService>();
builder.Services.AddScoped<AnnouncementFeedService>();
...
// Готовые логотипы для клубов, у которых своего ещё нет. Отдаются как обычные картинки, потому что
// `LogoUrl` потребляют и приложение игрока, и оболочка игрового ПК — им нужен настоящий URL, а не
// внутренний идентификатор пресета.
...
app.MapPlayerTournamentEndpoints();
...
organizations.MapBranchSearchEndpoints();
