  ...
  return (
    ...
    >
      {route.kind === 'overview' ? <OverviewScreen state={organizationMetrics} billing={billingMetrics} />
        ...
        : route.kind === 'organizations' || route.kind === 'organization' ? <OrganizationsScreen client={client} selectedOrganizationId={route.kind === 'organization' ? route.organizationId : null} initialInvite={readInitialInvite()} onOpenOrganization={id => openOrganization(id)} onCloseOrganization={() => navigate({ kind: 'organizations', query: '', status: 'all', plan: 'all', sort: 'attention' })} onCreateOrganization={() => navigate({ kind: 'organizationNew' })} />
        : <UnavailableScreen />}
    </AppShell>
  );
}
