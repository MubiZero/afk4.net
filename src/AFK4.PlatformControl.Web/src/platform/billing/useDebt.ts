...
export type DebtState =
  ...
  | { status: 'ready'; data: DebtRow[]; retry: () => void };

type Loadable = Pick<DebtApi, 'listDebt'>;

export function useDebt(client: Loadable): DebtState {
  const [tick, setTick] = useState(0);
  const [state, setState] = useState<{ status: 'loading' | 'error' | 'ready'; data?: DebtRow[]; message?: string }>({ status: 'loading' });
