...
describe('PlatformApiClient', () => {
  ...
  it('throws PlatformApiError with parsed body on failure', async () => {
    ...
    await expect(client.organizations.listOrganizations()).rejects.toMatchObject({
      status: 400,
  ...
  it('signs out and clears the session when the refresh attempt fails', async () => {
    ...
    await expect(client.organizations.listOrganizations()).rejects.toBeInstanceOf(PlatformApiError);
    expect(client.getSession()).toBeNull();
    expect(onSessionChanged).toHaveBeenCalledWith(null);
  });

  it('lists owner invites for a organization and returns the masked summary payload', async () => {
    const listBody = [
      {
    ...
    expect(lastCall[0]).toBe('http://localhost/api/platform/organizations/22222222-2222-2222-2222-222222222222/organization-owner-invitations');
