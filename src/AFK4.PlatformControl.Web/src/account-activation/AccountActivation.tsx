...
export function AccountActivation({ client, initialCode, kind }: AccountActivationProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!isWellFormedPin(password)) {
      return setError(t('auth.accept.error.passwordLength', { min: PIN_LENGTH }));
    }
    ...
    <Frame title={t(copy.title)} subtitle={t(copy.subtitle)}>
      <form className="auth-form" onSubmit={handleSubmit}>
        ...
        <Field label={t('auth.field.password')} htmlFor="accept-password">
          <Input id="accept-password" type="password" autoComplete="new-password" inputMode="numeric" maxLength={PIN_LENGTH} value={password} onChange={event => setPassword(keepPinDigits(event.target.value))} disabled={isSubmitting} required />
        </Field>
        <Field label={t('auth.accept.field.confirmPassword')} htmlFor="accept-confirm-password">
          <Input id="accept-confirm-password" type="password" autoComplete="new-password" inputMode="numeric" maxLength={PIN_LENGTH} value={confirmPassword} onChange={event => setConfirmPassword(keepPinDigits(event.target.value))} disabled={isSubmitting} required />
        </Field>
