...
4. иначе клуба нет, и эндпоинт, которому клуб нужен, отвечает `409 club_not_selected`.

Так `AFK4.Customer.Web` и Flutter-приложение продолжают работать без единой правки.

**Тесты вперёд**

- `PlatformPersonTokenServiceTests` — выдача/обновление/валидация, одноразовость refresh,
  ротация, отзыв при `IsActive = false`.
- `PlayerAuthenticationContextTests` — все четыре ветки выбора клуба выше, включая
  `409 club_not_selected`.
- `MeEndpointTests` — `GET /api/me` отдаёт личность и список клубов; в списке для каждого клуба
  название, остаток, придержано, долг, стаж; **и ни одного поля про чужие клубы в чужих ответах**.
- Совместимость: существующие `PlayerAuthenticationEndpointTests`, `PlayerCodeSignInEndpointTests`,
  `PortalReadsEndpointTests`, `PortalWritesEndpointTests` должны пройти **без правок** — если
  пришлось править, значит сломали контракт.

**Файлы**

- Новые: `Identity/IPlatformPersonTokenService.cs`, `Identity/OpaquePlatformPersonTokenService.cs`,
  `Identity/PlatformPersonContext.cs`, `Identity/IPlayerClubAccountResolver.cs`,
  `Identity/EfPlayerClubAccountResolver.cs`, `Endpoints/MeEndpoints.cs`,
  `Shared.Contracts/Players/MeDto.cs` (`MeDto(PersonDto Person, IReadOnlyList<MyClubDto> Clubs)`).
- Правки: `Identity/PlayerContext.cs` (+`PlatformPersonId`, `PlayerAccountId` становится
  nullable), `Identity/PlayerAuthenticationMiddleware.cs`, `Identity/OpaquePlayerTokenService.cs`
  (валидация старых токенов сохраняется до истечения 30-дневных refresh),
  `Endpoints/AuthEndpoints.cs`, `Program.cs` (DI).
...
### Ф3. Первое действие открывает счёт
...
1. **Переход токена на личность (Ф2).** Ошибка разлогинивает всех игроков разом. Защита:
   закреплённый клуб в токене, старые токены валидны до истечения, и требование, чтобы
   существующие тесты входа прошли **без правок**.
