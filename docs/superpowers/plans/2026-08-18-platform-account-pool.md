# Волна 1 — фундамент «общего котла платформы»

Дата: 2026-08-18. Статус: Ф1–Ф10 сделаны, включая доводку сервера под клиентов, выбор зала
в приложении игрока и разбор ревью волны от 21.08.2026; Ф5.1 и Ф11 отменены 20.08.2026.

Источники решения: решение владельца от 18.08.2026 о трёх слоях (личность/PIN/репутация —
платформенные; деньги, кешбэк, стаж, долг, чеки, локальные запреты — клубные), два концепта
интерфейса (приложение игрока и сторона стойки) и собранная из них очередь бэкенда на 29 работ.
Волна 1 — это семь работ, без которых модель не собирается вовсе.

Общего кошелька платформы не будет: у каждого клуба своя касса и отчётность. Клуб не заводит
человека — он получает доступ к уже существующей записи в момент первого действия.

**Правка от 20.08.2026: у проекта нет живых пользователей — ни одного игрока, ни одной боевой
базы.** Поэтому всё, что писалось ради плавного перехода и совместимости с существующими данными,
вычеркнуто из этого плана и удалено из кода: переносить нечего и переводить некого. Вычеркнуты
раздел 4 (перенос существующих данных) целиком, фазы Ф5.1 (переход по PIN) и Ф11 (разбор находок),
таблица находок `platform_identity_migration_findings`, SQL-перенос внутри миграции, показатель
`pin_adoption` и критерии «30 дней / 90% задавших PIN». Ниже эти места помечены как отменённые —
чтобы через месяц никто не искал в коде машинерию, которой там нет. Сам котёл (личность, связь,
счета, регистрация, сетевой PIN) остаётся ровно таким, как описан.

---

## 0. Что уже есть в коде (проверено чтением, не предположения)

Прежде чем планировать, я сверил семь пунктов с кодом. Четыре утверждения из постановки задачи
оказались неточными — это меняет объём работ, и лучше знать это до, а не после.

| Утверждение | Как на самом деле |
|---|---|
| «Заморозки нет ни в модели, ни в словах» | **Есть и работает.** `src/AFK4.Platform.Api/Reservations/ReservationHold.cs`: холд — отрицательная запись журнала `EntryType = "reservation_hold"`, связь с бронью через `Reason`, снятие — реверс с `ReversesLedgerEntryId`, идемпотентное. Причины снятия: `cancelled`, `seated`, `no_show`. Тесты: `tests/AFK4.Platform.Api.Tests/ReservationHoldTests.cs`. |
| «Неявка не разобрана» | Разбирается фоном: `ReservationNoShowRunner` + `ReservationNoShowHostedService`, grace 15 минут. Но **деньги при неявке всегда возвращаются** — удержания в пользу клуба нет, и «не приехал» изображается отменой с `CancelReason = "no-show"`, отдельного состояния нет. |
| «Авто/ручное подтверждение надо сделать» | Авто-подтверждение уже есть, но **зашито в код**: `EfReservationService.ShouldAutoConfirmOnlineAsync` — «кошелёк > 0 и нет долга». Плюс любая бронь с выбранным тарифом подтверждается автоматически всегда. Настройки филиала нет вообще. |
| «PIN хранится отдельно» | PIN — это `PlayerCredentialEntity.PasswordHash` (тот же ASP.NET `PasswordHasher`, что и пароль), запись 1:1 с клубным `PlayerAccountEntity`, то есть **PIN сегодня клубный по факту хранения**. Блокировка после 5 ошибок на 15 минут уже есть. |

Ещё три факта, которые определяют форму работ:

- **Денег как колонки не существует нигде.** Баланс и долг — это `SUM` по `ledger_entries`
  (`Billing/LedgerBalanceProjector.cs`). Холд уже вычтен из баланса, потому что он и есть
  отрицательная запись. Значит «третья сумма» — это не новая сущность денег, а **новое число в
  сводке**, и это радикально дешевле, чем кажется из концепта.
- **Токен игрока привязан к организации.** `OpaquePlayerTokenService.ValidateAsync` возвращает
  `PlayerContext(PlayerAccountId, OrganizationId, PhoneVerified)`; вход
  `POST /api/public/player/sign-in/code` принимает `organizationId`. Приложение уже выбирает клуб
  первым экраном. Перевод входа на личность — это и есть самая рискованная часть волны.
- **Одна миграция = одна ветка.** 108 миграций и один `PlatformDbContextModelSnapshot.cs`
  (4714 строк) на весь проект, один `PlatformDbContext`. Параллельные миграции гарантированно
  дают конфликт в снапшоте.

---

## 1. Целевая модель данных

```
platform_persons                  ← личность: телефон, имя, язык, PIN, сетевой запрет
      │ 1
      │
      │ N  (PlatformPersonId, nullable)
player_accounts                   ← связь человека с клубом + держатель клубного счёта
      │
      └── ledger_entries          ← деньги клуба: остаток, долг, холд (как есть сегодня)
```

Ключевые решения:

1. **`PlayerAccountEntity` не переименовывается и не разрезается.** Он и есть «связь и счёт».
   Добавляется одна колонка `PlatformPersonId uuid NULL`. Это сохраняет ~40 маршрутов `/api/me`,
   весь биллинг и всю операторскую часть работающими.
2. **`PlatformPersonId` остаётся nullable навсегда.** Записи, заведённые на стойке без телефона,
   — это не переходный мусор, а нормальный случай: гость без приложения. Они живут как чисто
   клубные и подшиваются к личности в тот день, когда человек подтвердит номер.
3. **Токен выдаётся личности, а клуб выбирается запросом.** У токена есть необязательный
   «закреплённый клуб» — так старые клиенты продолжают работать без единой правки (см. фазу 2).
4. **Холд остаётся отрицательной записью журнала.** Отдельного счёта `hold` не заводим: сегодня
   любая проверка «хватает ли денег» автоматически видит холд, а с отдельным счётом каждую из
   девяти таких проверок пришлось бы переписывать, и каждая — это потенциальное «списали дважды».

---

## 2. Одна миграция на всю волну

**Имя:** `AddPlatformAccountPool`.
**Команда:** `dotnet ef migrations add AddPlatformAccountPool --project src/AFK4.Platform.Api --startup-project src/AFK4.Platform.Api`.
**Файлы:** `src/AFK4.Platform.Api/Data/Migrations/<ts>_AddPlatformAccountPool.cs`, `.Designer.cs`,
обновлённый `PlatformDbContextModelSnapshot.cs`.

Миграция создаётся **целиком в фазе 1**, до того как написана хоть одна строка бизнес-логики
остальных шести пунктов. Это единственный способ совместить «фазы размером с коммит» с «одной
миграцией»: схема проектируется вперёд, дальнейшие фазы только пишут код.

Состав, в порядке применения:

| # | Что | Зачем |
|---|---|---|
| 1 | `platform_persons`: `PlatformPersonId`, `PhoneNumber varchar(32) NOT NULL`, `DisplayName varchar(160)`, `PreferredLocale varchar(16) NULL`, `PhoneVerifiedAtUtc`, `PinHash varchar(512) NULL`, `PinSetAtUtc NULL`, `PinFailedCount int`, `PinLockedUntilUtc NULL`, `NetworkBanAtUtc NULL`, `NetworkBanReason varchar(500) NULL`, `IsActive bool`, `CreatedAtUtc`, `UpdatedAtUtc`. Уникальный индекс по `PhoneNumber`. | Пункт 1, 3, 4 |
| 2 | `platform_person_access_tokens`, `platform_person_refresh_tokens` — по образцу `player_access_tokens`/`player_refresh_tokens`: `TokenHash bytea`, `PinnedOrganizationId uuid NULL`, `ExpiresAtUtc`, `RevokedAtUtc`. | Фаза 2 |
| 3 | `platform_phone_otps`: `Phone varchar(32)`, `Purpose int`, `CodeHash`, `CreatedAtUtc`, `ExpiresAtUtc`, `AttemptCount`, `ConsumedAtUtc`. Индекс `(Phone, Purpose, CreatedAtUtc)`. | Пункт 3: у незнакомого номера нет `PlayerAccountId`, а сегодняшний `player_phone_otps` требует его обязательно |
| 4 | `player_accounts.PlatformPersonId uuid NULL` + индекс; `player_accounts.CreatedFromApp bool NOT NULL DEFAULT false`; **уникальный** индекс `(PlatformPersonId, OrganizationId) WHERE PlatformPersonId IS NOT NULL` | Пункт 1: у человека в одном клубе ровно один счёт — это и есть защита от второй копии на уровне БД, а не на уровне надежды |
| 5 | `branch_booking_settings`: `BranchId` (PK), `OrganizationId`, `AcceptanceMode varchar(16)`, `RespondWithinMinutes int`, `RequirePrepaymentFromNewGuests bool`, `MaxActiveReservationsForNewGuests int`, `RegularAfterVisits int`, `HoldSeatAfterStartMinutes int`, `KeepPrepaymentOnNoShow bool`, `UpdatedAtUtc`, `UpdatedByStaffUserId` | Пункт 6 |
| 6 | `reservations.RespondByUtc timestamptz NULL`, `reservations.ConfirmedAtUtc timestamptz NULL` | Пункт 7 |
| 7 | `platform_reputation_snapshots`: `PlatformPersonId` (PK), `NetworkVisits int`, `NetworkNoShows int`, `CalculatedAtUtc` | Пункт 2 — защита от вычисления соседа по времени (см. раздел 5) |
| ~~8~~ | ~~`platform_identity_migration_findings`~~ | **Отменено 20.08.2026:** находки бывают только у переноса, а переносить нечего |
| ~~9~~ | ~~SQL-перенос данных `DO $cutover$ ... $cutover$`~~ | **Отменено 20.08.2026:** живых баз нет, переносить нечего |

`Down` откатывает пункты 1–7 в обратном порядке. **`Up` ничего не удаляет и ничего не
переписывает в существующих таблицах** — только добавляет.

Миграция перегенерирована 20.08.2026 штатно (`dotnet ef migrations remove` → `add`) после удаления
пунктов 8 и 9, поэтому её файл называется `20260820053713_AddPlatformAccountPool.cs`, а снапшот
собран из модели, а не подправлен руками.

---

## 3. Фазы

Каждая фаза — один коммит на своей ветке от `main`. Тесты пишутся первыми: AGENTS.md требует TDD
для доменных правил, контрактов и поведения эндпоинтов, а здесь всё три сразу.

Только **фаза 1** трогает миграции и `ModelSnapshot.cs`. Все остальные — нет.

*(Прежнее правило «Ф5 и Ф5.1 — один коммит и один релиз» отменено 20.08.2026 вместе с Ф5.1:
дня, в который старый PIN перестал работать, не будет — старых PIN не существует.)*

---

### Ф1. Схема и одна миграция

**Тесты вперёд**

- `tests/AFK4.Platform.Api.Tests/Migrations/PlatformAccountPoolMigrationTests.cs` — по образцу
  `SeparatePlatformAndOrganizationIdentityMigrationTests`:
  - `[MigrationPostgresFact]` (атрибут с `public const string EnvironmentVariable`, чтобы
    автоматически попасть под охрану `AFK4_REQUIRE_POSTGRES_TESTS`);
  - миграция накатывается на чистую схему настоящей PostgreSQL;
  - `Down` возвращает схему к исходной — ни одной новой таблицы и ни одной новой колонки.
  *(Проверки склейки телефонов, находок и неизменности `ledger_entries` отменены 20.08.2026
  вместе с самим переносом: склеивать нечего.)*
- `tests/AFK4.Platform.Api.Tests/Data/PlatformPersonSchemaTests.cs` — round-trip сущностей,
  уникальность телефона, уникальность пары `(PlatformPersonId, OrganizationId)`.

**Файлы**

- Новые: `Data/PlatformPersonEntity.cs`, `Data/PlatformPersonAccessTokenEntity.cs`,
  `Data/PlatformPersonRefreshTokenEntity.cs`, `Data/PlatformPhoneOtpEntity.cs`,
  `Data/BranchBookingSettingsEntity.cs`, `Data/PlatformReputationSnapshotEntity.cs`.
- Правки: `Data/PlayerAccountEntity.cs` (+`PlatformPersonId`), `Data/ReservationEntity.cs`
  (+`RespondByUtc`, `ConfirmedAtUtc`), `Data/PlatformDbContext.cs` (DbSet-ы и маппинг),
  миграция + снапшот.

**Проверка:** `dotnet build src/AFK4.Platform.Api`; `dotnet test tests/AFK4.Platform.Api.Tests
--filter FullyQualifiedName~Migrations|FullyQualifiedName~PlatformPersonSchema`. Postgres-тесты
обязаны реально бежать: без строки подключения они молча пропустятся, а пропущенный
Postgres-тест здесь равен непроверенной миграции.

---

### Ф2. Личность в токене и `/api/me` — **сделано**

Самая рискованная фаза волны: ошибка здесь разлогинивает всех игроков.

**Решение по совместимости.** Токен выдаётся личности, но несёт необязательный
`PinnedOrganizationId`. Порядок определения «текущего клуба» в
`PlayerAuthenticationMiddleware`:

1. заголовок `X-AFK4-Organization` (новые клиенты);
2. иначе `PinnedOrganizationId` токена (старые клиенты — вход по-прежнему принимает
   `organizationId` и закрепляет его);
3. иначе, если у человека ровно одна связь с клубом — она;
4. иначе клуба нет, и запрос, которому клуб нужен, получает `409 club_not_selected`.

Так `AFK4.Customer.Web` и Flutter-приложение продолжают работать без единой правки.

**Два контекста вместо nullable — принято при реализации, отменяет прежнее решение.**
План предполагал сделать `PlayerContext.PlayerAccountId` необязательным. При столкновении с кодом
это оказалось худшим из двух вариантов: `PlayerAccountId` читается примерно в сорока местах десяти
файлов эндпоинтов, и каждое — место, где ошибка выдаёт чужой кошелёк. Сорок мест, где можно
ошибиться деньгами, — это не рефакторинг, а мина.

Вместо этого контекстов стало два, и они отвечают на разные вопросы:

| | Отвечает на вопрос | Когда есть |
|---|---|---|
| `PlatformPersonContext` | «кто этот человек» | всегда, пока токен личности жив |
| `PlayerContext` | «чей это счёт и чьи это деньги» | только когда клуб определён |

`PlayerContext.PlayerAccountId` **остаётся обязательным**, `PlayerContext` дополнительно несёт
`PlatformPersonId`. Ни один эндпоинт не изменился. Вопрос «в каком клубе» решается один раз в
`PlayerAuthenticationMiddleware`, он же и отвечает `409 club_not_selected` — одно место вместо
сорока. Единственный маршрут, работающий без клуба, — `GET /api/me`: он клубы и перечисляет.

Названный, но чужой клуб получает тот же `409`; нечитаемый заголовок — `400 invalid_organization`.
Подставить вместо названного клуба другой нельзя: это показало бы человеку чужой кошелёк там, где
он ждал свой.

**Тесты (написаны, зелёные)**

- `Identity/PlatformPersonTokenServiceTests` — выдача, одноразовость и ротация refresh, отзыв при
  `IsActive = false` **на самой личности**; отдельно: закрытая клубом карточка не закрывает
  человеку вход в соседние клубы.
- `Identity/PlayerAuthenticationContextTests` — все четыре ветки выбора клуба, `409` на «клубов
  несколько, ни один не назван», `409` на чужой клуб, `400` на нечитаемый заголовок, `401` без токена.
- `Identity/PlayerClientCompatibilityTests` — ходит ровно теми запросами, что настоящий
  Flutter-клиент (`organizationId` в теле входа, дальше только `Authorization`, продление полем
  `refreshToken`), и проверяет, что вход действительно пошёл по новой дороге. Плюс: счёт, ещё не
  подшитый к личности, входит по-старому.
- `Identity/MeEndpointTests` — `GET /api/me` отдаёт личность и список клубов (название, остаток,
  придержано, долг, стаж по каждому), снятый холд перестаёт держаться, чужих клубов в ответе нет
  ни одним полем, PIN не утекает даже в сыром JSON.
- Совместимость: все существующие тесты прошли **без единой правки**.

**Файлы (как получилось)**

- Новые: `Identity/IPlatformPersonTokenService.cs`, `Identity/OpaquePlatformPersonTokenService.cs`,
  `Identity/PlatformPersonContext.cs` (контекст + аксессор), `Identity/IPlayerClubAccountResolver.cs`,
  `Identity/EfPlayerClubAccountResolver.cs`, `Endpoints/MeEndpoints.cs`,
  `Shared.Contracts/Players/MeDto.cs` (`MeDto(MePersonDto Person, IReadOnlyList<MyClubDto> Clubs)`).
- Правки: `Identity/PlayerContext.cs` (+`PlatformPersonId` необязательным параметром — все прежние
  места конструирования не тронуты), `Identity/PlayerAuthenticationMiddleware.cs`,
  `Identity/EfPlayerPhoneSignInService.cs` (вход выдаёт токен личности, если счёт к ней подшит),
  `Endpoints/AuthEndpoints.cs` (продление обслуживает оба вида токенов одним маршрутом),
  `Program.cs` (DI), `Billing/LedgerBalanceProjector.cs` (см. ниже).
- `Identity/OpaquePlayerTokenService.cs` **править не пришлось**: старые токены продолжают
  валидироваться сами, достаточно порядка проверок в middleware.

> **Досрочная находка для Ф7.** Третье число кошелька понадобилось уже здесь — `MyClubDto` без него
> неполон. В `Billing/LedgerBalanceProjector.cs` появился `GetClubBalancesAsync` (остаток,
> придержано, долг) и запись `ClubBalances`; существующая `GetWalletSummaryAsync` переведена на
> него. **Ф7 обязана расширять этот метод, а не заводить второй источник «придержано»** — два
> определения одной суммы разъедутся на первом же исправлении. Смысл `WalletBalance` при этом не
> тронут: холд как был вычтен из остатка, так и остался (риск 4).

**Проверка:** `dotnet test tests/AFK4.Platform.Api.Tests` целиком — фаза трогает
аутентификацию, то есть блок «cross-module» из AGENTS.md.

---

### Ф3. Первое действие открывает счёт — **сделано**

**Правило:** бронь, пополнение или посадка в клубе, где связи ещё нет, создают связь и счёт с
нулём **внутри той же транзакции**, идемпотентно, и склеиваются с карточкой, которую оператор
завёл руками по тому же телефону.

**Тесты вперёд**

- `PlayerClubMembershipServiceTests`:
  - два параллельных вызова с одним `IdempotencyKey` дают одну связь (Postgres-тест на гонку, по
    образцу `Reservations/PostgresBookingCapacityConcurrencyTests.cs`);
  - если в клубе уже есть `PlayerAccount` с этим телефоном и `PlatformPersonId IS NULL` —
    он **подшивается**, а не дублируется, и его деньги остаются на месте;
  - если счёт есть и уже подшит — возвращается он же;
  - счёт создаётся с нулём: ни одной записи журнала не появляется;
  - у нового счёта проставлен признак происхождения «из приложения» (для стойки).
- `MeReservationFirstActionEndpointTests` — бронь в незнакомом клубе проходит и не отвечает
  «игрок не найден».

**Файлы**

- Новые: `Players/IPlayerClubMembershipService.cs`, `Players/EfPlayerClubMembershipService.cs`.
- Правки: `Endpoints/PlayerSelfServiceEndpoints.cs` (бронь, группа, пополнение, старт сессии —
  вместо `accessor.Current.PlayerAccountId` вызов `membership.EnsureAsync(personId, orgId,
  branchId, idempotencyKey, ct)`). `Data/PlayerAccountEntity.cs` уже расширен в Ф1: пометку
  «из приложения» несёт `CreatedFromApp` — она нужна стойке, но игроку не показывается, это
  кухня клуба.

**Как получилось на самом деле**

- Маршрут, на котором счёт может открыться, **помечается явно**:
  `.OpensClubAccount()` (`Identity/PlayerFirstActionRoutes.cs`). Помечены четыре: бронь, групповая
  бронь, намерение пополнить, посадка за ПК. Middleware пропускает такой запрос без счёта, если
  клуб назван; всё остальное по-прежнему получает `409 club_not_selected`. Вариант «заводить счёт
  в middleware для всех подряд» отвергнут: пролистывание чужой витрины завело бы стойке список
  призраков вместо гостей.
- `PlatformPersonContext` получил `SelectedOrganizationId`, а `IPlayerClubAccountResolver` теперь
  возвращает `PlayerClubSelection(OrganizationId, Account)`: клуб без счёта — это нормальное
  состояние, из которого и вырастает первое действие.
- **`IdempotencyKey` в сигнатуре не нужен и не заведён.** Идентичность создаваемого — это сама
  пара `(PlatformPersonId, OrganizationId)`, и её держит уникальный индекс. Отдельный ключ добавил
  бы второй способ сказать то же самое, а вместе с ним и способ сказать это по-разному.
- **Счёт открывается последним шагом перед делом**, после проверки прав клуба, подтверждения
  номера и валидации запроса. Иначе каждая отклонённая попытка оставляла бы клубу карточку гостя,
  который так и не пришёл.
- **Известное ограничение — закрыто 21.08.2026.** У клуба с несколькими филиалами первое действие
  отвечало `branch_required`, а назвать филиал было нечем: у сети с двумя филиалами человек упирался
  в тупик на первой же брони. Закрыто ровно так, как обещано, — необязательным полем `BranchId` в
  `CreatePlayerReservationRequest`, `CreatePlayerReservationGroupRequest` и
  `PlayerTopUpIntentRequest`; оно доезжает до `OpenClubAccountIfNeededAsync` и дальше до
  `EnsureAsync`. У клуба с одним филиалом ничего не изменилось, клиент без поля работает как
  раньше, чужой филиал отвергается `branch_not_found` и счёта не открывает. Посадка за ПК как
  брала филиал из привязки машины, так и берёт: человек стоит у конкретного ПК, и спрашивать его
  об этом было бы странно. Гадать филиал за человека по-прежнему нельзя: он придёт в один, а счёт
  окажется в другом, и в отчётах клуба это будет выглядеть как два разных гостя.
- **Заметно живому оператору:** в списке клиентов появляются карточки людей, которые в клубе
  физически не были, — с признаком `CreatedFromApp`. Это и есть замысел («клуб не заводит
  человека»), но стойке это видно, и вкладка «Гости из приложения» из волны 2 нужна именно за этим.

> Уникальный индекс `(PlatformPersonId, OrganizationId)` из Ф1 — это то, что делает
> идемпотентность настоящей: при гонке вторая вставка падает на индексе и код перечитывает
> существующую связь, ровно как `EfBillingCommandService.ExecuteInTransactionAsync` уже делает
> для денежных команд.

**Проверка:** focused-тесты + Postgres-тест на гонку.

---

### Ф4. Самостоятельная регистрация — **сделано**

Сегодня записи заводит админ (`POST branches/{id}/players`), а вход по SMS работает только для
номера, который кто-то уже внёс. Человек, скачавший приложение дома, упирается в стену.

**PIN при регистрации не спрашивается** (решение владельца). Регистрация — это ровно два поля:
имя и язык, как на экране 06 концепта. PIN задаётся позже и в тот момент, когда он впервые
нужен: при первой посадке за ПК или из профиля. Лишний шаг на входе стоит дороже, чем экран
«задайте PIN» ровно в нужную секунду.

**Тесты вперёд**

- `PlatformRegistrationEndpointTests` (18 тестов, зелёные):
  - `POST /api/public/register/start` отвечает **побайтово одинаково** для знакомого и
    незнакомого номера (сравнение полных тел ответа, не только статусов) и в обоих случаях
    действительно шлёт SMS;
  - `POST /api/public/register/confirm` с кодом на незнакомый номер создаёт личность и возвращает
    токен; на знакомый — **входит** в существующую, не создавая второй и не трогая её клубы;
  - `PATCH /api/me` (имя, язык) под токеном — **вместо задуманного
    `POST /api/public/register/profile`**: профиль человека уже живёт на `/api/me`, и второй
    маршрут к тем же двум полям был бы вторым способом сказать то же самое;
  - лимит: с одного номера больше `MaxSendsPerHour` кодов не уходит **даже если номер
    незнакомый** — счётчик входа привязан к `PlayerAccountId`, которого у незнакомца нет;
  - исчерпанный лимит и кулдаун выглядят снаружи ровно как отправленный код;
  - через час счётчик отпускает: номер не выгорает навсегда.
- `PhoneEnumerationTests` — переборный сценарий: 50 номеров подряд с одного IP, проверяем, что
  различить «есть аккаунт / нет» по ответу нельзя. Часть попыток при этом упирается в лимит по
  IP и получает `429` — обе половины каждой пары стоят рядом, поэтому равенство ответов
  проверяется и на `200`, и на `429`.

**Как получилось на самом деле**

- **Просьба прислать код не ищет личность вовсе.** Не «ищет осторожно» — не ищет: в
  `StartAsync` нет ни одного запроса к `platform_persons`. Так неотличимость держится устройством
  кода, а не аккуратностью следующего автора. Цена названа честно: язык SMS берётся общий
  (`NotificationOptions.DefaultLocale`), потому что узнать язык человека можно только найдя его.
  В SMS едет один код и ничего больше, так что цена невелика.
- **Регистрация и вход — один маршрут.** Разделить их значило бы сказать звонящему, знаком нам
  его номер. Поэтому `/register/confirm` на знакомый номер просто впускает.
- **Вход `/api/public/player/sign-in/code` не тронут.** План предполагал переучить его на поиск
  по `platform_persons.PhoneNumber` и сделать `organizationId` необязательным. Этого не
  потребовалось: дверь без клуба — это и есть регистрация, а старый маршрут остаётся тем, чем был,
  для клиентов, которые про личность ничего не знают. Один маршрут вместо двух изменённых.
- **Токен личности научился жить без клуба.** `IPlatformPersonTokenService` отдаёт
  `PlatformPersonSessionResponse`: первые восемь полей — дословно `PlayerSignInResponse`, поэтому
  старый клиент читает ответ как читал; сверх них `PlatformPersonId`, `PreferredLocale` и
  `ProfileCompleted` («спрошены ли имя и язык» решает сервер, а не клиент). `PlayerAccountId` и
  `OrganizationId` стали необязательными — так выглядит человек, зарегистрировавшийся дома.
- **Клуб закрепляется за токеном, только когда он один.** При двух и более выбор делает человек
  заголовком `X-AFK4-Organization`: подставить клуб за него значит показать чужой кошелёк там, где
  он ждал свой. Продление (`RefreshAsync`) по той же причине **сохраняет** закрепление, а не
  подбирает клуб заново.
- **Отказ закрытой личности честный (`403 account_disabled`), и это не утечка:** до него доходит
  только тот, кто уже доказал владение номером кодом из SMS.
- **Список языков переехал в `Shared.Contracts/Localization/SupportedLocales.cs`.** Хардкод
  `new[] { "ru", "en", "tg" }` уже лежал в `BranchSettingsEndpoints`; второй такой же рядом
  разъехался бы с первым на первом же исправлении. `PATCH /api/me` отказывает незнакомому языку:
  тихо превратить его в русский значит соврать человеку, что он выбрал.
- **Известное ограничение (наследство Ф3, не новое):** карточка, заведённая оператором по телефону
  до того, как человек зарегистрировался, при регистрации к личности не подшивается — она подошьётся первым действием
  человека в этом клубе. До того `/api/me` её не покажет.

**Файлы (как получилось)**

- Новые: `Identity/IPlatformRegistrationService.cs`, `Identity/EfPlatformRegistrationService.cs`,
  `Identity/PhoneOtp/PhoneKeyedOtpStore.cs`, `Endpoints/PlatformRegistrationEndpoints.cs`,
  `Shared.Contracts/Identity/RegistrationContracts.cs`,
  `Shared.Contracts/Localization/SupportedLocales.cs`.
- Правки: `Identity/IPlatformPersonTokenService.cs` и
  `Identity/OpaquePlatformPersonTokenService.cs` (сессия без клуба, закрепление сохраняется при
  продлении), `Endpoints/MeEndpoints.cs` (`PATCH /api/me`), `Endpoints/AuthEndpoints.cs`
  (продление отдаёт расширенную сессию), `Identity/EfPlayerPhoneSignInService.cs` (тело ответа
  старого маршрута оставлено прежним), `Endpoints/BranchSettingsEndpoints.cs` (общий список
  языков), `Program.cs` (DI + политика лимитов `register-public`).
- `register-public` — своя ручка на 10 запросов в минуту с IP, а не общая `player-public`: это
  единственный маршрут, где чужой номер получает SMS за счёт клубов, и прикручивать его надо
  уметь, не трогая остальной край игрока.

**Проверка:** `dotnet test tests/AFK4.Platform.Api.Tests` — 2200 пройдено, 0 пропущено, 0 упало
(было 2181 до фазы); `dotnet test tests/AFK4.Shared.Contracts.Tests` — 141 пройдено.

---

### Ф5. PIN принадлежит игроку и работает во всей сети — **сделано**

**Тесты вперёд**

- `MePinEndpointTests`:
  - `PUT /api/me/pin` задаёт PIN (4–8 цифр, только цифры — сегодня валидация это `Length >= 4`
    без проверки на цифры и без верхней границы);
  - смена PIN — тот же маршрут и ничего сверх токена приложения: старый PIN не спрашивается и
    SMS-кода не требует. Спросить старое значило бы запереть выход ровно тому, кто его забыл
    (решение владельца от 20.08.2026);
  - маршрут работает у человека, у которого ещё нет ни одного клуба: PIN принадлежит личности, а
    не счёту, и задаётся обычно до первого визита;
  - сервер PIN **никогда не отдаёт** — в `MePersonDto` есть только признак `PinSet`.
- `PlatformPinVerificationTests`:
  - `POST /api/public/player/sign-in` — тот самый маршрут, которым игровой ПК входит сегодня, —
    проверяет **сетевой** PIN личности и открывает связь с клубом, если её нет (переиспользует Ф3);
  - после 5 ошибок — блокировка на 15 минут (`PinFailedCount`, `PinLockedUntilUtc`), ответ
    одинаковый для «нет такого номера», «PIN не задан», «PIN неверен» и «личность закрыта»;
  - **клубный `PlayerCredentialEntity.PasswordHash` не проверяется никогда** — ни как основной
    путь, ни как запасной. Решение владельца: старые PIN сброшены всем, значит мёртвы с первой
    минуты, а не «пока живут».
- `OperatorPinRouteRetirementTests`: `POST branches/{id}/players/{id}/pin` возвращает
  `409 { code: "pin_owned_by_player" }` **всегда**, с заголовком `Deprecation`, и пишет аудит
  на попытку.

**Почему PIN проверяет старый маршрут, а не новый `pin/verify` под device-учёткой**

Прежний план отдавал проверку PIN новому маршруту, который ходит под учёткой устройства. Проверка
кода показала, что такой учётки у оболочки нет: секрет `X-AFK4-Device-Credential` живёт в процессе
Agent Service, а Player Shell знает `OrganizationId`/`BranchId`/`DeviceId` из именованного канала,
но доказать их API не может. Значит новый маршрут потребовал бы проводить вход через Agent Service
и в день выката сломал бы каждую оболочку, установленную в поле. Поэтому проверка сетевого PIN
встаёт на существующий `POST /api/public/player/sign-in`: форма запроса та же (клуб, телефон,
PIN), форма ответа — надмножество прежней, и ни одна установка в поле не замечает перемены.

Цена названа честно: PIN проверяется публичным маршрутом, а не только с клубного ПК. Держат его
те же 10 запросов в минуту с IP и блокировка на 15 минут после пяти ошибок — но блокировка теперь
сетевая, и пять чужих неверных попыток на четверть часа лишают человека самопосадки во всех клубах
сразу, а не в одном. Раньше цена была той же по форме и меньше по охвату.

**Операторский маршрут установки PIN просто не существует.** Клуб не вправе задавать сетевой
пароль: иначе админ одного клуба получает вход от чужого имени в чужих клубах.

*Задумано было мягче — 409 `pin_owned_by_player` с заголовком `Deprecation` ради установленных в
поле версий Organization Admin. **Отменено 20.08.2026:** установок в поле нет, доживать нечему.
Маршрут `POST branches/{id}/players/{id}/pin` удалён вместе с контрактом `SetPlayerPinRequest`,
записью в аудит `players.pin.set` и своим единственным потребителем в Organization Admin (пункт
меню «PIN клиента», модалка, клиент API, строки локализации).* Чтение
`PlayerCredentialEntity.PasswordHash` уже удалено; сама колонка дропается миграцией волны 2.

**Файлы**

- Новые: `Identity/IPlatformPinService.cs`, `Identity/EfPlatformPinService.cs`,
  `Shared.Contracts/Identity/PinContracts.cs`.
- Правки: `Endpoints/MeEndpoints.cs` (`PUT /api/me/pin` — PIN живёт там же, где личность, а не в
  клубном `PlayerSelfServiceEndpoints`), `Identity/PlayerAuthenticationMiddleware.cs` (маршрут PIN
  работает без выбранного клуба), `Endpoints/AuthEndpoints.cs` (вход за ПК проверяет сетевой PIN),
  `Endpoints/PlayerManagementEndpoints.cs:144` (отказ + `Deprecation` + аудит).
- Удаляется: `Identity/PlayerCredentialService.cs` и `IPlayerCredentialService.cs` — после
  перевода входа на личность у клубного хеша не остаётся ни одного читателя, а сервис без
  читателя опаснее отсутствующего: он выглядит рабочей дверью. Заодно уходит его настоящий баг —
  поиск по `p.PhoneNumber == request.PhoneNumber` дословно, без `PhoneNumberNormalizer`.
  Сама колонка `PlayerCredentialEntity.PasswordHash` остаётся до миграции волны 2.

**Как получилось на самом деле**

- **Клубный хеш умер, а строка `player_credentials` — нет.** Читателей `PasswordHash` не осталось
  ни одного, но сама строка держит клубную отметку «номер подтверждён»
  (`EfPlayerPhoneSignInService`, `EfPlayerPhoneVerificationService`), поэтому волна 2 дропает
  колонку, а не таблицу.
- **Подтверждение номера теперь поднимает флаг и у личности.** Найдено переводом входа: `POST
  /api/me/phone/confirm` ставил `PhoneVerified` только на клубной карточке, а профиль читает
  личность — человек подтверждал номер и тут же видел «не подтверждён». Вход по коду так делал с
  Ф4, подтверждение — нет. Флаг поднимается, только если номер личности и номер карточки совпали:
  чужой номер объявлять подтверждённым нельзя, а смена номера — не эта волна.
- **`PlayerSignInRequest` получил необязательный `BranchId`.** Он нужен ровно в одном случае:
  многофилиальный клуб, в котором у человека ещё нет счёта. Клиенты, которые филиала не называют,
  работают как работали.
- **Пять неверных попыток запирают личность на 15 минут во всех клубах сразу**, и выйти из
  блокировки человек может сам — новый PIN в приложении её снимает. Иначе наказан тот, кого
  подбирали, а не тот, кто подбирал.
- **19 тестовых файлов переехали с клубного PIN на сетевой** через общий
  `tests/.../PlayerPinTestData.cs`. По дороге вскрылось, что почти все сиды строили телефон из
  шестнадцатеричных символов Guid: со старым дословным поиском это работало, а с нормализацией
  E.164 такой номер отвергается. Тесты проходили случайно.

**Проверка:** `dotnet test tests/AFK4.Platform.Api.Tests` — 2226 пройдено, 0 пропущено, 0 упало
(было 2200 до фазы); `dotnet test tests/AFK4.Shared.Contracts.Tests` — 141 пройдено;
`bun test` в `src/AFK4.Player.Shell.Web` — 59 пройдено.

---

### ~~Ф5.1 Переход: PIN задаётся в приложении~~ — **отменено 20.08.2026**

**Причина: переводить некого.** У проекта нет ни одного живого игрока, поэтому дня, в который
«старый PIN перестал работать», не существует, и вся машинерия плавного перехода была бы мёртвым
весом с первого дня. Из кода удалены и в этом плане вычеркнуты:

- разовая рассылка пуша `player.pin_migration` вместе с маршрутом
  `POST /api/platform/announcements/pin-migration/push`, шаблонами на трёх языках и записью в
  аудит `platform.announcements.pin_migration.push`;
- показатель `pin_adoption` вместе с маршрутом `GET /api/platform/analytics/pin-adoption`,
  `PinAdoptionReader` и `PinAdoptionDto`;
- критерии конца перехода «30 дней» и «≥ 90% задавших PIN» — считать нечего и закрывать нечего;
- объявление «PIN изменился» на экране входа Player Shell.

**Что из этой фазы осталось и почему.** Всё, что было не про переход, а про продукт, живёт дальше
в Ф5:

- PIN задаётся и меняется **в приложении**, в профиле, без старого PIN и без кодов
  (`PUT /api/me/pin`). Забыл — задал новый там же.
- **Человека без смартфона сажает администратор.** Поток «Посадить гостя» постоянный. Цена
  названа владельцем прямо: такой человек теряет самопосадку и подходит к стойке каждый раз.
- На экране входа за ПК поле называется «PIN», а не «Пароль», и рядом стоит **постоянная**
  подсказка о том, где PIN задаётся. Это не объявление о переходе, а ответ на вопрос «а где мне
  его взять» — он будет актуален и через год.
- Отказ во входе один на все причины и одинаков для любого номера: иначе по экрану игрового ПК
  можно проверять, у кого в сети есть аккаунт. Это свойство входа, а не переходного периода, и
  оно проверяется тестом `LoginScreen.test.tsx`.
- **Клуб PIN не назначает.** Маршрут `POST branches/{id}/players/{id}/pin` не отвечает 409 с
  `Deprecation`, как было задумано на время перехода, — **его просто нет**. Вместе с ним удалён
  его единственный потребитель: пункт «PIN клиента» в Organization Admin, модалка ввода, клиент
  API и восемь строк локализации.
- `PlatformPhoneOtpPurpose` содержит одно значение `Registration`: `PcSignIn`, `PinReset` и
  `SignIn` удалены — сценариев, которые их звали, нет.

---

### Ф6. Настройки приёма гостей у филиала — **сделано**

**Тесты вперёд**

- `BranchBookingSettingsEndpointTests`: `GET/PUT
  /api/organizations/{orgId}/branches/{branchId}/booking-settings`, право
  `ManageBranchSettings`, аудит на изменение, значения по умолчанию для филиала без строки
  (`auto`, 15 мин, предоплата с новых включена, 1 бронь, 3 визита, 20 мин держим место,
  предоплата при неявке **не** удерживается).
- `PlayerBookingRulesEndpointTests`: `GET /api/me/branches/{branchId}/booking-rules` отдаёт
  игроку режим, срок ответа, нужна ли **ему** предоплата (посчитано сервером по его визитам в
  этом филиале), сколько броней ему доступно, сколько держат место. Ни одного поля про других
  игроков.
- `EfReservationAcceptanceModeTests`: режим `off` → отказ `booking_disabled`; `manual` → бронь
  становится `pending` даже при полном кошельке; `auto` → как сегодня; лимит активных броней
  для новичка; порог визитов снимает лимит и предоплату.

**Файлы**

- Новые: `Branches/IBranchBookingSettingsService.cs`,
  `Branches/EfBranchBookingSettingsService.cs`, `Branches/BranchBookingSettingsDefaults.cs`,
  `Shared.Contracts/Branches/BranchBookingSettingsDto.cs`,
  `Shared.Contracts/Branches/UpdateBranchBookingSettingsRequest.cs`,
  `Shared.Contracts/Reservations/PlayerBookingRulesDto.cs`.
- Правки: `Endpoints/BranchSettingsEndpoints.cs`, `Endpoints/PlayerCatalogEndpoints.cs`,
  `Reservations/EfReservationService.cs` — `ShouldAutoConfirmOnlineAsync` заменён чтением
  настроек филиала (прежнее «кошелёк > 0 и нет долга» стало **поведением режима `auto`**, а не
  законом природы), `Audit/AuditActionNames.cs` (+`UpdateBookingSettings`).

> Флаг `PlatformFeatureNames.OnlineBooking` остаётся тем, чем был, — рубильником платформы
> для всей организации. Настройка филиала — это решение клуба. Смешивать их нельзя: сегодня
> владелец не может выключить брони в одном филиале, а платформа не должна лезть в решение клуба.

**Как сделано (20.08.2026), отличия от плана выше**

- Схему фаза не трогала: таблица `branch_booking_settings` заведена ещё в Ф1, а дефолты живут
  в коде. Строки настроек у филиала может не быть никогда — это норма, а не пробел.
- Появился `Reservations/PlayerBookingRules.cs`, которого в списке файлов не было: **один**
  расчёт «что настройки филиала значат для этого игрока» на две стороны — на приём брони и на
  ответ приложению. Две копии разошлись бы в первый же месяц, и игрок читал бы «доступна одна
  бронь», получая отказ на первой же.
- Появился `Shared.Contracts/Branches/BranchBookingAcceptanceModes.cs`: три режима — значения
  транспорта, их шлёт админка и читает приложение, поэтому им место рядом с DTO.
- Отказы по решению клуба (`booking_disabled`, `prepayment_required`,
  `active_reservation_limit`) отвечают **409**, а не 400: это состояние, а не кривой запрос.
- **Бронь на компанию считается одной**, а не по местам: потолок «одна бронь» не должен
  запрещать новичку прийти вчетвером.
- Визит — законченная сессия в этом филиале. Считаются именно они, а не брони: намерение
  показывает бронь, а знакомым клубу человека делает приход.
- `booking-settings` **не** отданы режиму поддержки платформы (в отличие от соседнего
  `branches/{id}/settings`) ни на чтение, ни на запись: принимать ли брони и на каких условиях
  — решение клуба.
- `EfReservationOnlineAutoConfirmTests` удалён, его три случая переехали в
  `EfReservationAcceptanceModeTests`: правило одно, и дом у него один.
- Тестам, которые про другое (конфликты мест, вместимость, права, отмена), понадобился филиал
  «беру брони у кого угодно» — `BranchBookingSettingsTestData`. Иначе каждый из них проверял бы
  заодно и правила приёма гостей.

**Проверка:** `dotnet test tests/AFK4.Platform.Api.Tests` — 2243 пройдено, 0 упало, 0 пропущено.

---

### Ф7. Заморозка — третья сумма кошелька — **сделано**

**Тесты вперёд**

- `LedgerBalanceProjectorHeldTests`: `HeldMinorUnits` = сумма непогашенных `reservation_hold`;
  после снятия холда становится нулём; удержание за неявку в «придержано» не попадает;
  **`WalletBalance` не меняет смысла** — он как был, так и остаётся доступным остатком (это
  специально: девять мест в коде читают его как «сколько можно потратить», и любое изменение
  смысла — это класс ошибок «списали дважды»).
- `BillingContractSerializationTests` / `ShiftContractSerializationTests`: третье число появилось,
  первые два не поехали; величина удержаний отдельна от времени и товара.
- `ReservationNoShowRetentionTests`:
  - `KeepPrepaymentOnNoShow = false` → как сегодня, реверс, деньги вернулись, записи удержания нет;
  - `KeepPrepaymentOnNoShow = true` → реверс холда **плюс** списание
    `reservation_no_show_fee` на ту же сумму, итог по кошельку тот же, но в журнале видно
    «заморозка снята» и «удержано за неявку», а «придержано» обнуляется;
  - **удержанное — это выручка клуба** (решение владельца): запись несёт `ShiftId` открытой в
    филиале смены;
  - если в момент разбора неявки открытой смены нет, запись пишется с `ShiftId = null`
    (принятое допущение, раздел 7.2);
  - повторный прогон фоновой задачи ничего не дублирует;
  - `HoldSeatAfterStartMinutes` берётся из настроек филиала, а не из зашитых 15 минут; ноль минут
    — законная настройка «место не держим»;
  - бронь, у которой холд уже снят (отменена вручную), удержания не получает.
- `ShiftRevenueReportTests`: удержания видны отдельной величиной в `ShiftRevenueDto.Earned` и
  входят в итог; при `KeepPrepaymentOnNoShow = false` величина нулевая, и снятая заморозка
  (реверс) выручкой не становится.
- `PlayerLedgerProjectorTests`: `reservation_hold` и `reservation_no_show_fee` фильтруются
  (раньше `PlayerLedgerFilter.KnownEntryTypes` **не содержал даже существующий**
  `reservation_hold` — то есть выписку по холдам отфильтровать было нельзя, эндпоинт отвечал 400).
- `EfMoneyActionPolicyResolverTests`: реверсы холдов и удержания за неявку **не** попадают в
  суточный счётчик высокорисковых действий кассира.

**Файлы**

- Правки: `Billing/LedgerBalanceProjector.cs`, `Shared.Contracts/Billing/WalletSummaryDto.cs`
  (+`HeldBalance`), `Shared.Contracts/Players/PlayerDashboardDto.cs` (+`HeldBalance`),
  `Shared.Contracts/Billing/LedgerEntryTypeNames.cs` (+`ReservationHold`, `ReservationNoShowFee`
  — заодно `reservation_hold` переехал из `ReservationHold.EntryType` в общий список, где ему и
  место), `Players/PlayerLedgerProjector.cs`, `Players/PlayerDashboardProjector.cs`,
  `Reservations/ReservationHold.cs` (+`CreateNoShowFee`),
  `Reservations/ReservationNoShowRunner.cs`, `Reports/EfReportService.cs` (`BuildShiftRevenue`),
  `Shared.Contracts/Shifts/ShiftRevenueDto.cs` (`EarnedBreakdownDto` +`NoShow`).

**Как получилось на самом деле, отличия от плана выше**

- **Счётчик антифрода править не пришлось, и это проверено, а не предположено.** Риск 5 был
  реальным по формулировке (`MoneyActionEntryTypes.HighRisk` действительно содержит `reversal`),
  но снятие холда в счётчик не попадает по двум независимым причинам: суточный расход считается
  по парам «этот актор» + «эта открытая смена», а холд ставит и снимает самообслуживание
  (`actorStaffUserId = Guid.Empty`) и вне смены (`ShiftId = null`). Ни один список типов не
  тронут; свойство закреплено тестом, который гоняет настоящие `ReservationHold.Create` и
  `ReleaseAsync`, а не подставляет строку типа руками.
- **Второго места в отчётах не оказалось.** План называл `EfReportService.cs:321` вторым местом
  расчёта выручки. Это `GetGameplayTimeReportAsync` — отчёт по сессиям, и суммы он берёт только по
  записям с `SessionId`. У удержания за неявку сессии нет и быть не может, поэтому там менять
  нечего.
- **Протухший комментарий в `EfReservationService` про «отложенную функцию холдов» уже был снят**
  раньше по ходу волны — правка из списка файлов не понадобилась.
- **`ReservationNoShowOptions.Grace` удалён, а не переопределён.** Держать рядом зашитые
  15 минут и настройку филиала значило бы завести второй ответ на один вопрос. У класса остался
  только `TickInterval` — как часто просыпаться, а не сколько ждать.
- **Итог `Earned.Total` теперь включает удержания.** Иначе получились бы деньги, которые в журнале
  есть, а в отчёте нет (риск 8). В `Earned.Time` удержание не попадает — это не проданное время.
- **Дневного отчёта филиала по выручке в коде нет.** Допущение 6 обещает, что удержание с
  `ShiftId = null` «видно в дневном отчёте филиала»; на деле сегодня его видно в журнале и в
  выписке игрока, а единственный суточный отчёт (`GetOwnerDailySummaryAsync`) — это
  антифрод-сводка по сотрудникам, и удержанию там не место. Само допущение в силе: запись не
  теряется и не приписывается чужой смене. Дневной отчёт по выручке — отдельная работа.
- **Заметно живому человеку.** Игрок и оператор видят третье число в кошельке. Клуб со
  включённым удержанием теперь оставляет предоплату себе, а игрок видит в выписке две строки
  вместо одной («заморозка снята», «удержано за неявку»). И место после начала брони по
  умолчанию держится **двадцать минут вместо пятнадцати** — это значение по умолчанию из Ф6,
  теперь оно действительно работает. Экран кассы в Organization Admin новую величину пока не
  показывает — это Ф10.

**Проверка:** `dotnet test tests/AFK4.Platform.Api.Tests` — 2259 пройдено, 0 упало, 0 пропущено
(было 2243 до фазы); `dotnet test tests/AFK4.Shared.Contracts.Tests` — 144 пройдено (было 141).

---

### Ф8. Срок ответа на заявку — **сделано**

**Тесты вперёд**

- `ReservationRespondByTests`: при создании `pending`-заявки
  `RespondByUtc = min(now + branch.RespondWithinMinutes, StartsAtUtc)`; при подтверждении
  ставится `ConfirmedAtUtc`, `RespondByUtc` перестаёт значить что-либо; поле доезжает и в
  `ReservationDto`, и в `PlayerReservationDto` — иначе у двух админов на разных машинах будут
  разные цифры, а у игрока третья.
- `ReservationRequestExpiryRunnerTests`: истёкшая заявка снимается сама, холд возвращается
  **всегда** (истечение — это не неявка, клуб не ответил, удерживать не за что), повторный прогон
  идемпотентен, подтверждённые заявки не трогаются.
- `Reservations/PostgresRequestExpiryConcurrencyTests` — гонка «администратор подтверждает / фон
  снимает по сроку» на настоящей PostgreSQL: барьер заставляет обе стороны прочитать заявку до
  того, как любая из них запишет, и проверяется, что победитель ровно один.
- `Architecture/PeriodicJobRegistrationTests` и `Platform/PlatformHealthWatchJobTests` — новая
  задача унаследована от общей обёртки и объявлена в каталоге интервалов (тесты уже существовали
  и упали бы, если забыть).

**Файлы**

- Новые: `Reservations/ReservationRequestExpiryRunner.cs` (вместе с `ReservationRequestExpiryOptions`),
  `Reservations/ReservationRequestExpiryHostedService.cs`.
- Правки: `Reservations/EfReservationService.cs`, `Reservations/ReservationHold.cs`
  (+причина снятия `request_expired`), `Shared.Contracts/Reservations/ReservationDto.cs`,
  `Shared.Contracts/Reservations/PlayerReservationDto.cs`,
  `Endpoints/EndpointHelpers.Dtos.cs`, `Endpoints/PlayerSelfServiceEndpoints.cs`,
  `Platform/Health/PlatformJobNames.cs`, `Platform/Health/PlatformJobIntervalCatalog.cs`,
  `Program.cs`.

**Как получилось на самом деле, отличия от плана выше**

- **Гонку решает версия-сторож брони, а не новый механизм.** `ReservationEntity.Version` уже
  объявлен concurrency token, поэтому подтверждение и снятие по сроку спорят в самой базе: вторая
  по времени запись не находит строку в том виде, в каком её читала. Поэтому каждая заявка
  сохраняется отдельным `SaveChanges` — проигранный спор снимает с игры одну заявку, а не весь
  проход, и возврат денег откатывается вместе с отменой. Победивший админ получает обычный
  `409 version_conflict`, победивший таймер — отменённую заявку; «подтверждена и отменена
  одновременно» не бывает.
- **Срок получает любая заявка, ждущая ответа, — и из приложения, и заведённая на стойке.**
  Иначе брони из приложения жили бы по часам, а заведённые руками висели бы вечно.
- **`ConfirmedAtUtc` ставит и посадка.** Усадить человека — это тоже ответ клуба; без этой
  отметки посаженная заявка выглядела бы так, будто ей никто и не ответил.
- **Причина отмены — `request-expired`** (через дефис, как соседний `no-show`), а причина снятия
  заморозки в журнале — `request_expired`. Формы разные, потому что таковы соседи: причина отмены
  читается на экране оператора, причина снятия — в строке журнала.
- **Перенос брони на более раннее время тянет срок за собой:** `RespondByUtc` подрезается до
  нового начала, иначе обещание пережило бы саму бронь.
- **Заметно живому человеку.** Заявка в режиме «смотрит администратор» больше не висит вечно:
  через `RespondWithinMinutes` (по умолчанию 15 минут) она снимается сама, место освобождается, а
  замороженные деньги возвращаются игроку целиком — даже если филиал держит предоплату при
  неявке. Игрок и админ получают обе даты в ответах API; показывают их приложение и Organization
  Admin в Ф10.
- **Дневного лимита на срок нет и не нужно:** границы `RespondWithinMinutes` (5 минут — сутки)
  уже проверяет Ф6.

**Проверка:** `dotnet test tests/AFK4.Platform.Api.Tests` — 2278 пройдено, 0 упало, 0 пропущено
(было 2259 до фазы); `dotnet test tests/AFK4.Shared.Contracts.Tests` — 145 пройдено (было 144);
`dotnet build src/AFK4.OrganizationAdmin.App -p:EnableWindowsTargeting=true` — зелёная.

---

### Ф9. Репутация и приватность — **сделано**

Единственный пункт, который решение про котёл **создаёт**, а не убирает. Без него платформа
работает каналом утечки клиентской базы между конкурентами, и обнаружится это не в тестах.

**Решение владельца:** точное число из суточного снимка. Оператор видит «в сети 14 визитов,
неявок нет», цифра обновляется раз в сутки, и эта задержка и есть защита — не ведро «10–49»,
не живой счётчик.

**Что получилось**

- `PlayerReputationDto` — ровно четыре поля: `NetworkVisits`, `NetworkNoShows`, `NetworkBanned`,
  `CalculatedAtUtc`. Состав закреплён рефлексивным тестом и проверкой имён в JSON.
- Два маршрута под филиалом, оба помечены `ViewPlayers` и **не** помечены
  `AllowPlatformSupportAccess`:
  - `GET .../branches/{branchId}/players/reputation/{platformPersonId}` — для человека, с которым
    у клуба есть основание;
  - `POST .../branches/{branchId}/players/reputation/lookup` — спрос по точному номеру, номер
    едет телом, маршрут под политикой лимита `reputation-lookup` (20/мин на сотрудника).
- Основание: строка `player_accounts` этой организации с этой личностью **либо** живая заявка
  (`pending`/`confirmed`/`seated`) с её номером. Второе не сводится к первому: гостя, который
  позвонил на стойку, записывают одним телефоном, счёта у него ещё нет.
- Числа считает `ReputationSnapshotRunner` под `PlatformPeriodicJob` — тот же приём и тот же
  каталог, что у неявок и просрочки. Задание `reputation_snapshot` (сутки) добавлено в
  `PlatformJobNames.Watched` и в `PlatformJobIntervalCatalog`, поэтому за ним следит сторож
  здоровья наравне с остальными.
- Аудит `players.reputation.view` пишется на **сам факт чтения** — и на успех, и на отказ.

**Чем закреплены свойства приватности**

- `ReputationContractTests` (`tests/AFK4.Shared.Contracts.Tests`) — состав полей DTO и состав
  имён в JSON. Пятое поле не появится незаметно.
- `ReputationAccessTests.StrangerAndNobody_AnswerIdentically` — человек чужого клуба и
  несуществующий идентификатор отвечают побайтово одинаково.
- `ReputationAccessTests.UnknownNumberAndFreshPerson_AnswerIdentically` — незнакомый сети номер
  отвечает тем же, чем зарегистрированный без визитов.
- `ReputationAccessTests.PartialNumber_IsNotASearch` — огрызок номера не поиск, а `400`.
- `ReputationAccessTests.NumberProbing_RunsOutOfPermits` — перебор упирается в лимит маршрута.
- `ReputationAccessTests.PlatformSupportSession_DoesNotReachReputation` — поддержка платформы
  сюда не ходит.
- `ReputationAuditTests.LookupAudit_LooksTheSameForKnownAndUnknownNumbers` — свой аудит клуб
  читает сам, поэтому в записи о спросе по номеру нет идентификатора личности: он был бы той
  самой разницей, которую скрыл ответ.
- `ReputationSnapshotTests.TwoRequestsInARow_SurviveAVisitToTheClubNextDoor` — визит человека в
  соседний клуб не меняет ответ между двумя запросами.
- `ReputationSnapshotTests.AsOfTime_IsTheSameForEveryone` — «на когда посчитано» одинаково у
  ветерана, у застрявшей с прошлого прохода строки и у человека без строки вовсе.

**Разошлось с планом (и почему)**

- **Агрегат не возит́ся внутри карточек и списков**, поэтому `Endpoints/ReservationEndpoints.cs`
  не правился: репутация — единственное действие, которое пишется в аудит на чтение, и в списке
  заявок это означало бы запись про каждого, кого администратор просто пролистал. Карточка
  заявки и карточка клиента спрашивают тот же маршрут по идентификатору личности.

  **Дополнено при доводке админки (21.08.2026):** самого идентификатора в операторских
  проекциях не было, и спрашивать по нему было нечем — админка ходила спросом по номеру даже
  там, где основание очевидно, то есть писала телефон в аудит и тратила лимит 20/мин зря.
  `PlatformPersonId` теперь необязательным полем едет в `PlayerAccountDto`,
  `PlayerSearchResultDto` и `ReservationDto` (включая проекцию посаженной заявки, чтобы поле не
  исчезало после посадки); у карточки, не подшитой к личности, его нет. Тем же заходом доехала
  `CreatedFromApp` в обе клиентские проекции: список клиентов растёт без участия стойки, и это
  единственное, чем ей объяснить незнакомую строку. Маршруты репутации и их правила доступа не
  тронуты. Закреплено `OperatorPlayerIdentityProjectionTests` и тремя контрактными тестами.
- **`NetworkBanned` читается вживую**, а не из снимка: сетевой запрет не имеет права ждать сутки,
  а «где человек играет» он не выдаёт.
- **Отказ по правам — `403`, а не `404`.** Правило «оба случая `404`» из раздела 5 относится к
  паре «нет такой личности» / «нет основания у клуба»; отсутствие у сотрудника права
  `ViewPlayers` — факт про сам клуб, а не про человека, и отвечает как везде в проекте.
- **Спрос по точному номеру всегда `200`.** Это и есть неразличимость: `404` на незнакомый номер
  превратил бы маршрут в проверялку «есть ли такой человек в сети».
- **«На когда посчитано» берётся не из строки человека**, а как максимум по всей таблице снимков
  (пока в сети не пересчитано ничего — начало текущих суток). Личное время выдавало бы, есть ли
  у человека строка вообще.

**Файлы**

- Новые: `Players/IPlayerReputationService.cs`, `Players/EfPlayerReputationService.cs`,
  `Players/ReputationSnapshotRunner.cs`, `Players/ReputationSnapshotHostedService.cs`,
  `Shared.Contracts/Players/PlayerReputationDto.cs`.
- Правки: `Endpoints/PlayerManagementEndpoints.cs`, `Audit/AuditActionNames.cs`,
  `Platform/Health/PlatformJobNames.cs`, `Platform/Health/PlatformJobIntervalCatalog.cs`,
  `Program.cs`.

Источник чисел на волне 1: визиты — сессии в состоянии `ended`/`reconciled`, привязанные к счетам
одной личности; неявки — брони в состоянии `cancelled` с `CancelReason = "no-show"`. Отдельного
состояния `no_show` пока нет (это пункт 10 волны 2); когда оно появится, меняется один запрос в
`ReputationSnapshotRunner`, а не контракт.

**Заметно живому человеку.** У администратора появляется способ узнать про незнакомого гостя одно
число и только его; чужие клубы, даты и суммы недоступны ни одним полем. Игрок ничего не замечает:
маршрутов игрока фаза не трогает.

**Проверка:** `dotnet test tests/AFK4.Platform.Api.Tests` — 2300 пройдено, 0 упало, 0 пропущено
(было 2278 до фазы); `dotnet test tests/AFK4.Shared.Contracts.Tests` — 148 пройдено (было 145).

---

### Ф10. Клиенты не ломаются

Бэкенд-волна, но три клиента читают эти контракты, и «зелёный бэкенд при красном приложении» —
это не готово.

- `src/afk4_customer_app/lib/api/dto.dart` и `player_api_client.dart` — руками (генератора нет):
  `MeDto`, третье число в кошельке, `RespondByUtc`, правила приёма, PIN-маршруты. Плюс
  **экран профиля «задайте PIN»** — это единственная экранная работа, которая в волну входит:
  приложение — единственное место, где PIN задают. *(Экран-перехватчик «Задайте PIN» отменён
  20.08.2026 вместе с Ф5.1: перехватывать некого.)* Тесты:
  `src/afk4_customer_app/test/player_api_client_test.dart`.
  Проверка: `flutter test`, `flutter analyze`.
  **Плюс одна ветка, найденная в Ф2:** у клиента на `401` стоит автоматическая попытка продления,
  а на `409` — нет, поэтому `club_not_selected` для него сейчас тупик с общей ошибкой. Человеку с
  закреплённым клубом он недостижим, но достижим в одном случае: клуб закрыл карточку, пока человек
  был залогинен. Лечится в `player_api_client.dart` одной веткой — `409 club_not_selected` ведёт на
  экран выбора клуба, а не в «что-то пошло не так». На сервере костыля под это нет намеренно.
- `src/AFK4.Customer.Web/src/api/playerApi.ts` + `types.ts` — то же минимальным объёмом.
  Проверка: `bun test`, production-сборка.
- `src/AFK4.Player.Shell.Web` — экран входа за ПК: поле «PIN», постоянная подсказка о том, где
  PIN задаётся, и два пути при отказе («задайте PIN в приложении» · «позовите администратора»),
  одинаковые для любого номера. Сделано в Ф5 вместе с сервером.
- `src/AFK4.OrganizationAdmin.Web/src/api/clients/players.ts`, `.../reservations.ts` — агрегат
  репутации, срок ответа, настройки приёма; кнопка «PIN клиента» и её модалка удалены вместе с
  маршрутом (Ф5), а не заменены строкой-пояснением; новая величина удержаний в экране кассы
  (`ShiftRevenueDto`). Проверка: `bun test`.

Прочие экранные работы (полоса заявок, вкладка «Гости из приложения», профиль игрока) — **не в
этой волне**; здесь только то, без чего клиент падает, врёт или не даёт человеку задать PIN.

**Доводка сервера под клиентов (21.08.2026) — сделано**

Три недоделки, найденные при доводке клиентов; первые две обещаны Ф3 и Ф10, третья к волне
отношения не имеет.

1. **Филиал в первом действии.** Необязательный `BranchId` в брони, брони на компанию и намерении
   пополнить — см. Ф3 выше. Клиентам он теперь доступен, но не обязателен: приложение, веб игрока
   и админка правятся отдельной работой, и запрос без поля работает как раньше.
2. **Правила приёма — новичку.** `GET /api/me/branches/{branchId}/booking-rules` требовал открытого
   счёта в клубе, то есть молчал ровно перед тем, кому предоплата, ручной приём и потолок броней
   важнее всего, — перед человеком до первой брони. Маршрут получил вторую пометку рядом с
   `.OpensClubAccount()` — `.AllowsGuestWithoutClubAccount()` (`Identity/PlayerFirstActionRoutes.cs`):
   клуб назван, счёта нет, и это нормальное состояние, а не `409 club_not_selected`. Требование
   опознать человека осталось на месте: без токена ответ тот же `401`. Счёта чтение правил не
   открывает — иначе пролистывание витрины завело бы стойке карточки-призраки, — а числа гостю
   считаются честно как новому: ноль визитов, ноль активных броней (`PlayerBookingRules.EvaluateAsync`
   принимает `Guid?`).
3. **Карточка заявки после посадки** (пре-существующий дефект, не волна). `ReservationDto` собирали
   в двух местах, и посаженная заявка приходила без тарифа, суммы, валюты и времени подтверждения:
   вторая проекция этих полей просто не знала. Проекция сведена в одну —
   `Reservations/ReservationProjection.cs`, — и список заявок, и посаженная заявка приходят из неё.

**Проверка:** `dotnet test tests/AFK4.Platform.Api.Tests` — 2312 пройдено, 0 упало, 0 пропущено;
`tests/AFK4.Shared.Contracts.Tests` — 152 пройдено. Девять новых тестов проверены на красном до
правки.

**Выбор зала в приложении игрока (21.08.2026) — сделано**

Тупик закрыт с той стороны, где его видит человек: `src/afk4_customer_app`.

- **Залы приложение уже держит в руках.** Отдельного маршрута не понадобилось:
  `GET /api/public/organizations` отдаёт `places[]` с `branchId`, названием и адресом каждого
  зала, а выбранная сеть вместе с ними лежит в `SelectedOrganizationStore`. Это тот же ответ,
  из которого игрок выбирал сам клуб.
- **Один вопрос на два действия.** `lib/organization/branch_choice.dart`: зал помнит оболочка,
  спрашивают его лист брони и лист пополнения. У сети с одним залом вопроса нет — он и есть
  ответ, и уезжает сам; у игрока со счётом вопроса нет тем более: зал записан в счёте, и
  присланный его не переписывает. Пока зал не назван, кнопка действия погашена — сервер
  ответил бы тем самым `branch_required`.
- **`branchId` уходит** в `POST /api/me/reservations`, `.../reservations/group` и
  `.../wallet/top-up-intent`. Заглушка «У этой сети несколько залов — позвоните в клуб»
  (`customer.reservations.errBranch`) удалена; вместо неё `branch_required` и
  `branch_not_found` названы своими словами (`customer.branch.err*`).
- **Правила приёма — до счёта**, по названному залу: ровно то, ради чего маршрут получил
  `.AllowsGuestWithoutClubAccount()`. Новичок видит ручной приём и предоплату, ещё не начав
  заполнять форму.
- **Первое пополнение стало достижимым.** Раздел денег в клубе без счёта показывал только
  «Здесь вы ещё не играли», и открыть счёт пополнением было нечем — при том, что сервер это
  умеет. Теперь под запиской стоит «Пополнить».
- **Умолчания у выбора нет намеренно.** Про зал человека без счёта сервер не знает ничего —
  ни визитов, ни профиля у него в этом клубе ещё не существует, — и подставлять было бы
  гаданием. В пределах сеанса ответ помнится: второй лист не спрашивает то же самое.
- **Попутно найдено сквозным сценарием:** раздел денег не замечал открытия счёта (не было
  `didUpdateWidget`) и до перезапуска приложения оставался с вечно мерцающим скелетом кошелька.
  Починено там же.

**Проверка:** `flutter analyze` — 0 замечаний; `flutter test` — 364 пройдено (было 353);
`flutter test integration_test -d flutter-tester` — 1 пройден. Сквозной сценарий переписан на
сеть из двух залов и нового игрока: до брони клуб его не знает, зал называет он сам, и той же
бронью открывается счёт.

---

### Разбор ревью волны (21.08.2026) — сделано

Сквозная проверка всей волны перед вливанием нашла три места, где сделанное расходилось с
обещанным. Каждое закрыто тестом на красное до правки.

1. **Закрытая клубом карточка перестала быть закрытой.** Оператору на экране деактивации обещано
   дословно: «Денежные операции и вход на место станут недоступны». На деле человек с закрытой
   карточкой получал `200 OK` на пополнение: `PlayerAuthenticationMiddleware` ищет счёт с
   фильтром `IsActive`, не находит и пропускает запрос как первое действие, а
   `EfPlayerClubMembershipService` искал уже без этого фильтра — и отдавал ту же закрытую
   карточку как рабочую. До волны такой токен просто не проходил валидацию, то есть это был
   регресс, а не старое поведение. Теперь закрытая карточка отвечает
   `409 club_account_closed` и на первом действии, и на входе по PIN у ПК (там — тем же немым
   `401`, что и неверный PIN). Закрытая **ничейная** карточка с тем же номером тоже запрет:
   свежая рядом с ней обошла бы решение клуба через регистрацию в приложении. Живая ничейная
   карточка по-прежнему сильнее закрытой, а закрытая в одном клубе не трогает соседний.
2. **Игрок мог заплатить за молчание клуба.** Филиал с настройками «место не держим» и
   «предоплату при неявке удерживаем» штрафовал заявку, на которую клуб **ни разу не ответил**:
   `ReservationNoShowRunner` брал и `Pending`, а от неверного исхода спасал только порядок двух
   независимых фоновых раннеров. Это противоречило допущению 7 раздела 7.2 и вдобавок положило бы
   в будущую сетевую репутацию неявку, которой не было. Теперь неподтверждённая заявка
   закрывается как молчание клуба: полный возврат, причина `request-expired`, удержания нет ни
   при каких настройках. Штраф остался ровно там, где ему место, — у подтверждённой брони.
3. **Красный тест в `AFK4.Customer.Web`.** Правка общей строки `customer.reservations.respondOver`
   ради приложения оставила веб-тест ждать прежний текст, и `bun test` там после этого не гоняли.
   Тест сведён к общему каталогу — и заодно проверяет, что вместо отсчёта не показываются нули.

Попутно: `club_account_closed` доезжает до человека своими словами (`customer.club.errClosed`) в
приложении и в вебе — и в брони, и в пополнении. Общий ответ на 409 в листе брони звучит «Это
время уже занято», и закрытый счёт под ним выглядел бы враньём про причину.

**Проверка:** `dotnet test tests/AFK4.Platform.Api.Tests` — 2320 пройдено, 0 упало, 0 пропущено
(из них 31 на настоящей PostgreSQL); `tests/AFK4.Shared.Contracts.Tests` — 152 пройдено; сборка
решения целиком — 0 предупреждений; `bun run test` во всех веб-воркспейсах; `flutter analyze` —
0 замечаний, `flutter test` — 371, сквозной сценарий — 1.

**Найдено рядом и починено тем же заходом** (к волне отношения не имеет, но живёт в этом же
репозитории, а значит наше):

- **Красный тест входа в `src/AFK4.PlatformControl.Web`** — падал и на `main`. Тест проверял, что
  истёкшее окно подтверждения возвращает на шаг пароля с честной причиной, но выдавал срок,
  мёртвый уже в момент ответа сервера: экран кода при таком сроке живёт доли миллисекунды, и
  промежуточная проверка «мы были на экране кода» превращалась в гонку, которую тест всегда
  проигрывал. Сам компонент вёл себя правильно. Тест переписан на срок, который доживает до
  экрана кода и истекает уже под ним, — то есть проверяет ровно тот сценарий, ради которого
  `useChallengeExpiry` и написан.
- **Три воркспейса не гонялись в CI вовсе**: `src/AFK4.Player.Shell.Web` (59 тестов, включая тот,
  которым Ф5 доказывает одинаковый отказ на любой номер), `packages/tokens` (20) и `packages/ui`
  (5). Все три добавлены в `pr-verification.yml` — в прогон тестов, а Player Shell ещё и в сборку;
  сам он добавлен и в фильтр изменений, иначе правка только в нём не будила бы веб-джобу.

---

### ~~Ф11. Перенос существующих данных~~ — **отменено 20.08.2026**

Переносить нечего: живых баз у проекта нет. Вместе с фазой отменены маршруты разбора находок
(`GET /api/platform/identity-migration/findings`, `POST .../{findingId}/resolve`) — они так и не
появились в коде, и заводить их не нужно.

---

## ~~4. Миграция существующих данных~~ — **отменено 20.08.2026**

Весь раздел вычеркнут по одной причине: **у проекта нет ни одного живого игрока и ни одной боевой
базы**, поэтому склеивать по телефону нечего, находки писать не о чем, а предполётная проверка
считала бы нули.

Что было в разделе и чего теперь нет ни в плане, ни в коде: правила склейки по номеру; таблица
находок `platform_identity_migration_findings` с видами `duplicate_in_club`, `name_mismatch`,
`unusable_phone`; SQL-перенос `DO $cutover$` внутри миграции `AddPlatformAccountPool`;
предполётная проверка из пяти чисел; порядок применения на staging и проде; проверочный список
после применения; окно отката в сутки.

Единственное, что от раздела уцелело и переехало в Ф1, — требование проверять миграцию на
настоящей PostgreSQL: она обязана накатываться на чистую базу и откатываться начисто.

Новая база заводится миграциями с нуля, и первая же личность появляется штатным путём —
человек регистрируется сам (Ф4), а клуб открывает ему счёт первым действием (Ф3).

---

## 5. Безопасность и приватность

### Где именно стоит ограничение на репутацию

Не в интерфейсе. Три рубежа:

1. **В контракте.** `PlayerReputationDto` физически не имеет полей под названия клубов, даты и
   суммы, и это закреплено рефлексивным тестом. «Скрыто в UI» — не защита: операторское
   приложение ходит в тот же API, что и curl.
2. **В сервисе.** `EfPlayerReputationService` отдаёт агрегат только тому клубу, у которого есть
   основание: существующая связь с этим человеком либо живая заявка в его филиал. Иначе `404`,
   неотличимый от «нет такого человека».
3. **В снимке.** Агрегат читается из `platform_reputation_snapshots`, пересчитываемого раз в
   сутки. Это и есть защита от вычисления соседа: клуб, опрашивающий счётчик каждую минуту,
   иначе увидит `+1` ровно в тот момент, когда человек сел за ПК напротив, — и узнает, где он
   играет, не получив ни одного названия клуба. Суточная гранулярность делает такую корреляцию
   бессмысленной, а точность цифры («14 визитов») сохраняет.

### Что пишется в аудит

Действие `players.reputation.view` через существующий `IAuditRecordWriter`:
актор (сотрудник), организация, филиал, `TargetType = "platform_person"`, `TargetId`, исход
(`Succeeded` / `Denied`), и в `DetailsJson` — контекст обращения. Отказы пишутся тоже: перебор
виден только по отказам.

**Уточнено при реализации (Ф9):** у спроса по точному номеру `TargetId` **пуст всегда**, а номер
лежит в `DetailsJson`. Причина в том, что свой аудит клуб читает сам: заполненный идентификатор
личности у знакомого сети номера и пустой у незнакомого отличали бы их по собственному журналу
клуба — ровно ту разницу, которую скрыл ответ.

Аудит пишется на **сам факт запроса**, а не на изменение. Это отличается от остального аудита в
проекте и сделано намеренно: утечка здесь происходит через чтение.

### Какие ответы обязаны быть неразличимы

| Пара | Обязаны совпадать |
|---|---|
| «Код на знакомый номер» / «на незнакомый» | Тело, статус **и** факт отправки SMS. Свойство уже есть в `EfPlayerPhoneSignInService.StartAsync` и обязано пережить переход на регистрацию. Сейчас оно ещё и улучшается: раньше незнакомцу SMS не уходила, и разницу было видно по времени ответа |
| «Кулдаун активен» / «лимит исчерпан» / «SMS не ушла» на публичном входе | Всё то же `Sent`. Настоящие статусы отдаёт только `EfPlayerPhoneVerificationService`, где вызывающий уже аутентифицирован — ему скрывать нечего |
| «Неверный PIN» / «PIN не задан» / «нет такого номера» / «личность закрыта» на `POST /api/public/player/sign-in` | Один отказ, один статус (`401` без тела). Именно поэтому подсказку «PIN задаётся в приложении» даёт оболочка, а не сервер, и даёт её **всем одинаково**, а не по конкретному человеку (Ф5) |
| «Нет такой личности» / «нет права смотреть репутацию» | `404` в обоих случаях |
| Кривой номер | **Может** отличаться (`400 invalid_phone`): номер, который не может принадлежать никому, ничего не выдаёт |

### Как не дать перебирать номера

- IP-лимит `player-public` 10/мин уже есть; для регистрации добавляется отдельная политика.
- **Лимит по номеру, а не по аккаунту.** Сегодня `MaxSendsPerHour = 5` считается по
  `PlayerAccountId`, которого у незнакомого номера нет, — то есть после включения регистрации
  незнакомый номер получил бы неограниченную рассылку. Счётчик переезжает в
  `platform_phone_otps`, ключ — нормализованный телефон. Это одновременно защита от перебора и
  от SMS-бомбинга за счёт клуба.
- Поиск людей через платформу — **только точное совпадение нормализованного номера**, с лимитом
  и аудитом. Поиск по части номера остаётся тем, чем был: поиском по своим игрокам внутри клуба.
- Публичный каталог клубов (`GET /api/public/organizations`) не получает ни одного поля про
  людей.

### Что ещё нельзя сломать по дороге

- Клубный админ **не может** задать или сменить сетевой PIN (Ф5, маршрут отвечает `409` всегда) и
  не может снимать сетевой запрет. Попытка пишется в аудит.
- Проверка PIN не шлёт ни одной SMS и потому не тратит деньги клуба. Держат её лимит по IP
  (`player-public`, 10/мин) и блокировка личности на 15 минут после пяти ошибок — блокировка
  теперь сетевая, и это осознанная цена сетевого PIN (Ф5).
- `/api/me` не суммирует деньги по клубам ни в одном поле: `LedgerBalanceProjector` падает на
  смешанных валютах, и это правильно — общего кошелька нет, суммировать нечего.
- Токен личности отзывается при `IsActive = false` на самой личности, а не только на счёте.

---

## 6. Что можно параллелить после вливания волны 1

**Жёсткое ограничение, не зависящее от логики:** одна миграция — одна ветка. `ModelSnapshot.cs`
конфликтует всегда. Поэтому первым шагом волны 2 делается **одна общая схемная миграция**
(`AddBookingTruthfulness`: состояние `no_show` и удержанная сумма, код причины отказа,
происхождение сессии), и только после её вливания расходятся ветки.

После этого параллельно, разными агентами, без пересечения по файлам:

| Поток | Пункты волны 2 | Основные файлы |
|---|---|---|
| **A. Цена в приложении** | 08 (тариф, цена часа, зона в активной сессии), 17 (докуда действует тариф) | `Players/PlayerDashboardProjector.cs`, `Shared.Contracts/Players/ActiveSessionDto.cs`, `Tariffs/*` |
| **B. Код ПК на мониторе** | 09 | `Devices/*`, `Player.Shell*`, новый эндпоинт обмена кода |
| **C. Выписка игроку** | 14 (`GET /api/me/wallet/ledger`) | `Players/PlayerLedgerProjector.cs`, `Endpoints/PlayerSelfServiceEndpoints.cs` |
| **D. Реалтайм по броням** | 12 | существующий SignalR-хаб, `Reservations/EfReservationService.cs` (только точки публикации событий) |

**Обязаны идти одним потоком, последовательно:** пункты 10 (состояние «не приехал»), 11 (отказ с
причиной) и 13 (происхождение сессии) — все трое переписывают одну и ту же машину состояний в
`EfReservationService.cs` / `EfReservationSessionCoordinator.cs`. Три ветки в этом файле — это
гарантированный тройной конфликт и, что хуже, три разных представления о том, чем отличается
отмена от неявки.

**Обязаны ждать волну 1 целиком:** 16 (сетевой запрет против локального — нужна личность),
20 (свободные места в витрине — нужна публичная витрина с настройками приёма из Ф6),
22 (поиск людей в палитре — нужны правила доступа к репутации из Ф9).

Поток D пересекается с последовательным потоком по одному файлу — планировать его либо до, либо
после, но не одновременно.

---

## 7. Решения и допущения

### 7.1 Решения владельца (согласовано 2026-08-18)

1. **PIN задаётся и меняется только в приложении.** Входа на игровой ПК по коду из SMS не
   существует: SMS платные, а такой вход стал бы привычкой и постоянной статьёй расходов.
   Забывший PIN меняет его в приложении без кодов; человека без смартфона сажает администратор,
   и потерю самопосадки для него владелец принял сознательно.
   *(Прежний пункт «PIN сбрасывается всем» отменён 20.08.2026: сбрасывать нечего и некому —
   клубных PIN в живых базах не существует.)*
2. **PIN при регистрации не спрашивается.** Регистрация — имя и язык; PIN задаётся при первой
   посадке за ПК или из профиля.
3. ~~**Склейка по телефону — всегда.**~~ **Отменено 20.08.2026** вместе с переносом: склеивать
   нечего. Правило «кто владеет номером, тот владеет личностью» осталось в силе как свойство
   модели — телефон уникален в `platform_persons`, — но никакой разовой операции склейки нет.
4. **Репутация — точное число из суточного снимка.** Задержка и есть защита от вычисления
   соседа по динамике счётчика.
5. **Удержанная при неявке предоплата — выручка клуба**, с проводкой в кассовую смену и
   отдельной строкой в отчётах.

### 7.2 Допущения, которые я принял сам

Владелец их не подтверждал. Каждое — обратимое и дешёвое в отмене; если какое-то не нравится,
менять надо до начала фазы, где оно закреплено.

| # | Допущение | Почему так | Что переделывать при отказе |
|---|---|---|---|
| ~~1~~ | ~~Дубли одного телефона внутри одного клуба не сливаются автоматически~~ | **Отменено 20.08.2026:** дублей нет, потому что нет данных. Уникальный индекс `(PlatformPersonId, OrganizationId)` и так не даёт завести второй счёт | — |
| 2 | Репутацию вправе спросить клуб, у которого есть **связь с человеком или живая заявка** в его филиал; сверх этого — только точное совпадение номера | Иначе оператор пробивает любой номер и платформа становится справочником «кто где играет» | Одно правило в `EfPlayerReputationService` (Ф9) |
| 3 | «Предоплата только с новых» понимается буквально: завсегдатай бронирует без заморозки, и его неявка не стоит ему денег | Так написано на экране 06 концепта. Но это значит, что неявка своего бесплатна — а именно свои бронируют чаще всего | Настройка филиала (Ф6) + ветка в холде (Ф7) |
| 4 | Имя личности принадлежит человеку: `platform_persons.DisplayName` — единственное имя, которое видит оператор. `player_accounts.DisplayName` остаётся, но становится **клубной пометкой** («Фаррух с PS5»), а не вторым именем | Иначе админ переименовывает человека в чужих клубах, и это ровно та утечка контроля, ради которой всё разводится по слоям | `PATCH players` и проекции оператора (Ф2, Ф10) |
| 5 | Смена номера человеком не двигает клубные счета: связи держатся на `PlatformPersonId`, а не на телефоне | Иначе человек с новой SIM теряет деньги во всех клубах. Сам экран смены номера — вне волны 1 | Ничего: это следствие модели |
| 6 | Удержание за неявку вне открытой смены пишется с `ShiftId = null` — видно в дневном отчёте филиала, не видно ни в одном отчёте по смене | Ни одна смена за него не отвечала. Альтернатива — держать деньги замороженными до открытия клуба — хуже для игрока | Правило в `ReservationNoShowRunner` (Ф7) |
| 7 | Истечение срока ответа на заявку **всегда** возвращает деньги, даже при включённом удержании | Клуб не ответил — удерживать не за что. Удержание платит за неявку игрока, а не за молчание клуба | Ветка в `ReservationRequestExpiryRunner` (Ф8) |
| 8 | PIN — 4–8 цифр, только цифры | Сегодня валидация это `Length >= 4` без верхней границы и без проверки на цифры, то есть «PIN» мог быть паролем | Валидация в `EfPlatformPinService` (Ф5) |
| ~~9~~ | ~~Окно перехода по PIN — 30 дней, порог 90% задавших PIN~~ | **Отменено 20.08.2026** вместе с Ф5.1: перехода нет, мерить его конец нечем и незачем | — |
| 10 | Неявки для агрегата репутации считаются как `cancelled` + `CancelReason = "no-show"`, пока волна 2 не заведёт настоящее состояние `no_show` | Числа честные уже сейчас, а контракт от появления состояния не меняется | Один запрос в `ReputationSnapshotRunner` (Ф9) |


## 8. Самое рискованное

1. **Переход токена на личность (Ф2). — Закрыт.** Ошибка разлогинивала бы всех игроков разом.
   Защита сработала: закреплённый клуб в токене, старые токены валидны до истечения, существующие
   тесты входа прошли без единой правки.
   Отдельно отмечу главное: **вторая половина риска — «править сорок эндпоинтов, читающих
   `PlayerAccountId`» — закрыта не аккуратностью, а архитектурно.** Разделение на два контекста
   (`PlatformPersonContext` — кто человек, `PlayerContext` — чей счёт) убрало саму необходимость
   их трогать: сорока мест, где можно ошибиться деньгами, просто не появилось. Аккуратность
   масштабируется плохо, отсутствие правок — идеально.
2. ~~**День выката PIN (Ф5.1).**~~ **Риска нет с 20.08.2026:** дня, в который старый PIN
   перестал работать, не существует — нет ни одного человека со старым PIN. Осталось только
   постоянное правило продукта: **сервер, экран Player Shell и экран PIN в приложении едут
   одним релизом**, иначе самопосадку негде включить.
3. ~~**Склейка по телефону (Ф11).**~~ **Риска нет с 20.08.2026:** склейки не происходит.
   Опасность «два разных человека делят один номер» никуда не делась, но она теперь обычная и
   постоянная: телефон уникален в `platform_persons`, и кто владеет номером, тот и владеет
   личностью. Разовой необратимой операции, которую надо было бы откатывать снимком, нет.
4. **Смысл `WalletBalance`.** Соблазн сделать его «валовым» и вычитать холд отдельно
   огромен и ломает девять мест, включая единственную настоящую проверку достаточности средств
   в `EfWalletSettlementService.DebitAsync` (которая считает ещё и застейдженные записи
   `ChangeTracker`). Правило волны: **`WalletBalance` не меняет смысла ни при каких
   обстоятельствах**, третье число только добавляется.
5. **Реверсы холдов в счётчике антифрода. — Проверено в Ф7, чинить было нечего.**
   `MoneyActionEntryTypes.HighRisk` действительно содержит `reversal`, но суточный расход
   считается по паре «этот сотрудник» + «эта открытая смена», а холд ставит и снимает
   самообслуживание (`actorStaffUserId = Guid.Empty`) и вне смены (`ShiftId = null`). Ни одна
   снятая бронь лимит кассира не трогает; свойство закреплено тестом, а не рассуждением.
6. **Молча пропущенный Postgres-тест.** Миграция и гонки проверяются только на настоящем
   Postgres. Локальный прогон без строки подключения зелёный и ничего не доказывает; в CI это
   ловит `AFK4_REQUIRE_POSTGRES_TESTS=1`, но до CI надо дожить с правильным атрибутом
   (`public const string EnvironmentVariable` — иначе тест не попадёт под охрану).
7. **Одна миграция на всю волну означает, что фазу 1 нельзя делать частично.** Забытая колонка
   стоит второй миграции и конфликта снапшота. Список из девяти позиций в разделе 2 —
   исчерпывающий; если по ходу фаз 2–9 понадобилась десятая, это ошибка проектирования схемы, и
   лучше остановиться и дособрать фазу 1, чем завести вторую миграцию.
8. **Отчёт по выручке. — Закрыт в Ф7.** `earnedTime` в `BuildShiftRevenue` собирается по белому
   списку типов записей, и `reservation_no_show_fee` туда не добавлен. Чтобы удержания не стали
   деньгами, которых нет ни в одном отчёте, у них своя величина `Earned.NoShow`, и она входит
   в `Earned.Total`.

---

## 9. Как проверять

- Каждая фаза: `dotnet build src/AFK4.Platform.Api` + focused-тесты фазы.
- Фазы 1, 2, 5, 7 (миграция, аутентификация, PIN, деньги) — вся `tests/AFK4.Platform.Api.Tests`:
  требование AGENTS.md для миграций, auth-границ и денежных границ.
- Фазы, меняющие контракты (2, 4, 5, 6, 7, 8, 9): плюс `tests/AFK4.Shared.Contracts.Tests`.
- Фаза 5 дополнительно: `bun test` в `src/AFK4.Player.Shell.Web` — экран входа за ПК живёт
  там, и «отказ выглядит одинаково для любого номера» проверяется тестом, а не глазами.
- Фаза 7 дополнительно: `--filter FullyQualifiedName~Report` — удержания обязаны появиться в
  отчёте по смене, иначе они деньги-призраки.
- Фаза 10: `flutter test` + `flutter analyze` в `src/afk4_customer_app`, `bun test` в
  `AFK4.Customer.Web`, `AFK4.Player.Shell.Web` и `AFK4.OrganizationAdmin.Web`.
- Перед вливанием волны: полная сборка решения и полный прогон тестов — затронуты миграции,
  общие контракты и границы аутентификации.
- Postgres-тесты (миграция вперёд и назад, гонка при создании связи, гонка при брони) должны
  **реально выполниться**. Пропуск = провал, не «ну на CI прогонится».

**Проверки, которых тестами не бывает** — их делает человек на стенде перед выкатом на прод:

1. установка PIN в приложении и самопосадка за ПК с ним (Ф5) — на живом стенде, а не на моке;
2. посадка гостя оператором для человека без телефона — путь не сломан;
3. человек, зарегистрировавшийся сам и сходивший в два клуба, видит оба.

*(Пункты про пуш о смене правил, про людей «после переноса» и про 409 на кнопке «Задать PIN»
отменены 20.08.2026: переноса нет, кнопки нет, маршрута нет.)*

**Порядок релиза.** Сервер, экран входа Player Shell и экран PIN в приложении (Ф10) выкатываются
одним релизом: порознь они дают установку, в которой самопосадка есть на экране, но её негде
включить.

Ожидаемое суммарное покрытие новыми тестами: порядка 75–85 тестов, из них 3–4 на настоящем
Postgres.
