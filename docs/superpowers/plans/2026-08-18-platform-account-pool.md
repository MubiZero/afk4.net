...
### Ф5.1 Переход: PIN задаётся в приложении
...
**Проверка:** `dotnet test tests/AFK4.Platform.Api.Tests` целиком (затронуты границы
аутентификации) + `bun test` в `src/AFK4.Player.Shell.Web`. Плюс ручная проверка на стенде
(раздел 9): тест не докажет, что пуш дошёл и что человек понял экран.
