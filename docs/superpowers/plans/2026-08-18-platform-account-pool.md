...
Ф5 и Ф5.1 — **один коммит и один релиз**: порознь они дают день, в который старый PIN уже не
работает, а нового ещё негде задать (риск 2).
...
2. **Совместимость со старыми клиентами.** Organization Admin обновляется в том же релизе (Ф10):
   кнопка «Задать PIN» заменяется строкой «PIN задаёт игрок сам, в приложении». Старые установки
   в поле кнопку сохранят и получат понятный отказ — это хуже, чем
...
### Ф5.1 Переход: PIN задаётся в приложении — **сделано**

**Решение владельца от 20.08.2026 отменяет прежний замысел перехода.** Входа на игровой ПК по коду
из SMS не будет ни в каком виде — ни постоянного, ни спрятанного на месяц. Причина не техническая:
SMS платные, а вход по коду стал бы для людей основной привычкой, то есть не мостом через переход,
а постоянной статьёй расходов клубов. Всё, что было написано в прежней версии этого раздела про
`pc-login/code/*`, `PcCodeSignInTests` и поле «Придумайте PIN» после входа по коду, — закрыто
целиком, и следов в коде остаться не должно.

**Новая схема — «только приложение».**

- PIN задаётся и меняется **в приложении**, в профиле. Это ничего не стоит: человек уже вошёл.
- **Забыл PIN — задаёт новый там же**, без кодов, без администратора и без старого PIN. Спросить
  старый значило бы запереть выход ровно тому, кто за ним пришёл.
- Вышел из приложения и забыл PIN — сначала обычный вход по коду на номер. Это существующая дорога
  и редкая: токен живёт 30 дней, поэтому в неё упирается не каждый забывший, а только сменивший
  телефон или переустановивший приложение.
- **Человека без смартфона сажает администратор.** Поток «Посадить гостя» существует и не меняется.
  Цена решения названа владельцем прямо: такой человек теряет самопосадку и подходит к стойке
  каждый раз. Это выбор, а не недосмотр, и обходных путей для него мы не строим.
- Объявление о переходе идёт **пушем** на зарегистрированные устройства, а не рассылкой SMS.

**Что показывает Player Shell пришедшему со старым PIN.** Сервер отвечает на любую неудачу входа
одинаково — иначе по экрану игрового ПК можно проверять, у кого в сети есть аккаунт. Поэтому
объясняет не сервер, а сама оболочка, и объясняет **всем одинаково**, что бы ни было введено:

> **PIN изменился.** Теперь он один на все клубы сети, и задаёте его вы сами — в приложении,
> в профиле. **Нет приложения под рукой — позовите администратора**, он посадит вас за ПК.

Ни одной ветки «этому показать, тому не показать» в оболочке нет и быть не может: чтобы ветвиться,
оболочке пришлось бы узнать от сервера, знаком ли ему номер, — а он этого не говорит никому.

**Флага `pin_migration_notice` не будет — и это не упрощение, а следствие устройства флагов.**
Платформенные фичи (`PlatformFeatureNames` → `FeatureCatalog` → `IOrganizationEntitlements`)
включаются **клубу** и читаются только под токеном игрока (`GET /api/me/features`). Экран входа за
ПК — единственное место оболочки, где токена ещё нет, а заводить ради баннера публичный маршрут
«какие флаги включены» значит открыть анонимному звонящему кусок состояния платформы. Поэтому текст
перехода живёт в самой оболочке и уходит вместе с её обновлением в закрывающем коммите. Флаг без
потребителя — тот же мусор, что и `PlatformPhoneOtpPurpose.PcSignIn`, который этот раздел удаляет.

**Как игрок узнаёт заранее.**

- Пуш о смене правил — тем, у кого есть зарегистрированное устройство (`PlayerDeviceEntity`) и ещё
  не задан PIN. Механизма «объявление всем игрокам» в проекте нет: платформенные объявления
  адресованы клубам, а все существующие пуши — одному `PlayerAccountId`. Поэтому переход приносит
  один разовый маршрут рассылки (ниже) и уносит его закрывающим коммитом.
- Экран-перехватчик при открытии приложения: «Задайте PIN — он теперь один на все клубы».
  Пропускаемый, но возвращается, пока PIN не задан. Живёт в Ф10.
- Экран на ПК (выше) — канал, который застаёт человека ровно в момент нужды. Пуши и баннеры доходят
  до меньшинства, а к ПК приходят все.

**Что делает админ у стойки.**

- **Человек со смартфоном** — задаёт PIN в приложении сам, за десять секунд, стоя у ПК.
- **Человек без смартфона или с недоступным номером** — админ **сажает его сам**, как сегодня.
  PIN нужен только для самопосадки, а не для игры: без возможности играть не остаётся никто.
- В карточке клиента вместо кнопки «PIN клиента» — строка «PIN задаёт игрок сам, в приложении».
  Оператор PIN не видит и не назначает больше никогда.

**Что закрывается в конце перехода** (а что не закрывается):

| | |
|---|---|
| Закрывается | объясняющий текст на экране входа Player Shell; экран-перехватчик в приложении; разовый маршрут рассылки пуша; маршрут `POST branches/{id}/players/{id}/pin` и экран «PIN клиента»; колонка `PlayerCredentialEntity.PasswordHash` (миграцией волны 2) |
| **Не** закрывается | посадка гостя оператором — она постоянная и остаётся единственной дорогой для человека без смартфона |
...
Третьего условия — «доля самопосадок по SMS-коду ниже 20%» — больше нет: считать нечего, входа по
SMS не существует. Оставшийся критерий обязан быть измеримым, иначе у перехода нет конца, а вечного
режима совместимости мы как раз и не хотим. Показатель `pin_adoption` — отдельный маршрут в
`Endpoints/PlatformAnalyticsEndpoints.cs`, а не новое поле в `PlatformAnalyticsOverviewDto`:
обзорную сводку читает Platform Control, и подмешивать в контракт про деньги показатель про
переход значит просить чужой экран пережить чужое изменение.

**Файлы перехода**

- Новые: `Notifications/Templates/{ru,en,tg}/player.pin_migration.json`,
  `Identity/PinMigrationAnnouncer.cs`, `Platform/Analytics/PinAdoption.cs`.
- Правки: `Notifications/NotificationTemplateKeys.cs` (+`PlayerPinMigration`),
  `Endpoints/PlatformAnalyticsEndpoints.cs` (+`GET /api/platform/analytics/pin-adoption`),
  `Endpoints/PlatformAnnouncementEndpoints.cs` (+`POST /api/platform/announcements/pin-migration/push`),
  `Data/PlatformPhoneOtpEntity.cs` (**минус** `PcSignIn`),
  `src/AFK4.Player.Shell.Web/src/screens/LoginScreen.tsx` (объяснение и два пути).

**Как получилось на самом деле**

- **Флаг перехода не заводился** — по причине из раздела выше. Текст живёт в оболочке и уходит с
  её обновлением.
- **Рассылка — платформенный маршрут `POST /api/platform/announcements/pin-migration/push`**
  (право `ManageAnnouncements`, аудит). Адресаты: у кого есть зарегистрированное устройство и нет
  PIN. Ключ идемпотентности привязан к игроку, а не к нажатию: второе нажатие никого не будит
  второй раз.
- **Показатель — `GET /api/platform/analytics/pin-adoption`** (право `ViewPlatformHealth`), с
  двумя знаменателями: все, кто приходил за окно, и те из них, кто вообще может задать PIN.
  Гость, заведённый на стойке, к личности не подшит и PIN задать не может — включить его в
  знаменатель значило бы сделать порог 90% недостижимым по построению.
- **На экране входа за ПК поле «Пароль» стало полем «PIN».** Это и есть PIN, а называть вещь
  чужим именем — тот же обман, что и объяснять смену правил только тем, кого узнали.
- **Заодно удалён `PlatformPhoneOtpPurpose.PinReset`:** восстановление PIN по SMS отменено тем же
  решением владельца, и у значения не осталось сценария. `SignIn = 1` тоже стоит без потребителя
  с Ф4 (вход и регистрация делят один код `Registration`) — это наследство прежней фазы, и
  трогать его в этой я не стал.

**Проверка:** та же, что у Ф5 (это один коммит). Плюс ручная проверка на стенде (раздел 9): тест
не докажет, что пуш дошёл и что человек понял экран.
...
- `src/afk4_customer_app/lib/api/dto.dart` и `player_api_client.dart` — руками (генератора нет):
  ...
  **экран профиля «задайте PIN»** и экран-перехватчик из Ф5.1 — это единственная экранная
  работа, которая в волну входит, потому что без неё переход не начинается вовсе: приложение —
  единственное место, где PIN теперь задают. Тесты:
...
- `src/AFK4.Player.Shell.Web` — экран входа за ПК: объяснение про сменившийся PIN и два пути
  («задайте PIN в приложении» · «позовите администратора»), одинаковое для всех. Сделано в Ф5.1
  вместе с сервером: порознь эти две половины дают день без самопосадки — см. риск 2.
...
| **Клубные PIN** | **Не переносится ни один.** У всех личностей `PinHash IS NULL`, все задают PIN заново | Решение владельца. Клубный PIN назначал админ; промотировать его до сетевого — значит выдать админу вход от чужого имени в чужих клубах. Как при этом не оставить людей без самопосадки в день выката — целиком в Ф5.1 |
...
- контрольная выборка: один человек задаёт PIN в приложении и садится за ПК сам; второй, без
  смартфона, посажен администратором (проверка Ф5.1 на живом стенде, а не в тесте).
...
| «Неверный PIN» / «PIN не задан» / «нет такого номера» / «личность закрыта» на `POST /api/public/player/sign-in` | Один отказ, один статус (`401` без тела). Именно поэтому объяснение про смену правил PIN даёт оболочка, а не сервер, и даёт его **всем одинаково**, а не по конкретному человеку (Ф5.1) |
...
1. **PIN сбрасывается всем.** Ни один клубный PIN не переносится — в том числе в однозначном
   ...
   *(Дополнено 20.08.2026)* **PIN задаётся и меняется только в приложении.** Входа на игровой ПК
   по коду из SMS не существует: SMS платные, а такой вход стал бы привычкой и постоянной статьёй
   расходов. Забывший PIN меняет его в приложении без кодов; человека без смартфона сажает
   администратор, и потерю самопосадки для него владелец принял сознательно.
...
| 9 | Окно перехода по PIN — 30 дней, порог 90% задавших PIN | Числа выбраны как разумные, а не выведены из данных. Прежний второй порог (доля входов по SMS ниже 20%) отменён вместе с самим входом по SMS. Смотреть по факту на показателе `pin_adoption` | Одно число в `GET /api/platform/analytics/pin-adoption` (Ф5.1) |
...
2. **День выката PIN (Ф5.1).** Второй по величине риск после токена и единственный, который
   создан принятым решением: сброшены все PIN разом, а нового способа войти по SMS больше нет.
   Первые дни самопосадка держится на одном: у человека есть приложение и он задал в нём PIN.
   Все остальные идут к стойке — поток посадки оператором цел, но нагрузка на администратора в
   эти дни выше обычной, и клубы надо предупредить заранее. Правило: **сервер, экран Player Shell
   и экран PIN в приложении выкатываются одним релизом; порознь они дают день, в который старый
   PIN уже не работает, а новый негде задать.**
...
- Фаза 5.1 дополнительно: `bun test` в `src/AFK4.Player.Shell.Web` — экран входа за ПК живёт
  там, и «объяснение показывается всем одинаково» проверяется тестом, а не глазами.
...
1. установка PIN в приложении и самопосадка за ПК с ним (Ф5.1) — на живом стенде, а не на моке;
   плюс пуш о смене правил, дошедший до настоящего телефона;
...
**Порядок релиза.** Ф5 и Ф5.1 — один коммит и один релиз; экран PIN в приложении (Ф10) выкатывается
тем же релизом. Разделить их — значит устроить день, в который старый PIN уже не работает, а новый
негде задать.
