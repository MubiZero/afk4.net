...
> **Поправка после Ф2.** Ф3 планировалась в расчёте на то, что после Ф2 клубного счёта в контексте
> может не быть вовсе. По факту его там нет только тогда, когда клуб не определён, и такой запрос
> до эндпоинта не доходит — middleware отвечает `409 club_not_selected` раньше. Значит связь надо
> заводить не «вместо отсутствующего счёта в эндпоинте», а **до** этого отказа: в момент, когда
> клуб назван (заголовком или закреплением), связи с ним нет, а действие её оправдывает. Кто
> именно вызывает `EnsureAsync` — эндпоинт или middleware — решается в начале Ф3; вариант «в
> middleware для всех подряд» отвергнут заранее: чтение чужой витрины не повод заводить счёт.
