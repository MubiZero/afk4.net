...
1. **Сотрудники добавляются по номеру, а не скриптом** (вход в два шага — с 25.09.2026).
   `Управление` -> `Сотрудники и роли` -> `+ Сотрудник`: **номер телефона**, имя
   в смене и роли. Логин не спрашивается — им становятся цифры номера. Панель
   сразу показывает шестизначный **код первого входа**; он живёт сутки. SMS с
   тем же кодом уходит на номер, но код надёжнее передать самому.
   Сотрудник открывает Панель AFK4.net, вводит свой номер — и Панель сама
   спрашивает код первого входа, потом новый ПИН-код дважды, и пускает внутрь.
   Дальше он входит так же: номер, потом ПИН-код. Три неверных кода гасят код —
   руководитель добавляет сотрудника заново и видит новый.
   Код нужен потому, что номер не секрет: без него ПИН-код новому сотруднику
   назначил бы любой, кто знает его номер.
