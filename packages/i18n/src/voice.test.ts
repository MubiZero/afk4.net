...
// Таджикский держит своё «кэшбэк» — это его глоссарий, правило к нему не относится.
it('пишет «кэшбек» одним написанием по-русски', () => {
  const hits = Object.entries(messages.ru)
    .filter(([, value]) => /к[эе]шб[эе]к/i.test(value.replace(/кэшбек/gi, '')))
    .map(([key, value]) => `ru:${key} = "${value}"`);
  expect(hits).toEqual([]);
});
