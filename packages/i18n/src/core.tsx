...
/**
 * Каталог строк по языкам. Полный — у Панели, приложения и мастера; экрану ПК хватает своей части
 * (`catalogs/playerShell.ts`): весь каталог — больше мегабайта JS на холодный старт киоска.
 */
export type Catalog = Readonly<Record<Locale, Readonly<Record<string, string>>>>;
...
function resolveMessage(catalog: Catalog, locale: Locale, key: MessageKey): string {
  const direct = catalog[locale][key];
  if (direct !== undefined) return direct;
  for (const fb of LOCALE_FALLBACK[locale]) {
    const value = catalog[fb][key];
...
// Standalone translator for a fixed locale over a given catalog — the provider's t and
// createTranslator are both built on it.
export function createCatalogTranslator(catalog: Catalog, locale: Locale) {
  return (key: MessageKey, values?: Record<string, string | number>): string => {
    const message = resolveMessage(catalog, locale, key);
...
export function I18nCatalogProvider({
  ...
  const t = useMemo(() => createCatalogTranslator(catalog, locale), [catalog, locale]);
