...
- [Operator redesign — Этапы](operator-redesign-phase0-decisions.md) — Этапы 0/1/2/3/4 ЗАКРЫТЫ; **Касса (Этап 2) завершена** (S0 #109 + S1 #110 + S2 #111 + S3 #112 в main); впереди 5 Отчёты / 6 Управление; вкладки Кассы = `sales`(POS+Заказы)/`shift`/`journal`; currency = const-мапа `@afk4/money` (TJS→«с.», целые; разделитель тысяч = NBSP, отрицат. ASCII-минус); App.test отдельным `bun test`-прогоном; флак-урок: дренировать вторую волну рефетчей после nonce-бампа; паттерн «воркспейс как сегмент» (`embedded` корень `<section>` вместо `<main>`) — канон для слияний.
