...
- [Operator «Касса» (Этап 2)](operator-redesign-phase0-decisions.md) — **В РАБОТЕ**: **S0 каркас (PR #109) + S1 смена (PR #110) в main**. Канон: spec `docs/superpowers/specs/2026-06-24-operator-cash-design.md`. Раздел = воркспейс `cash` (шапка-якорь `CashShiftHeader`: статус смены + командная панель Открыть/Внести/Изъять/Закрыть → модалки + под-вкладки `sales`/`orders`/`shift`/`review`). S1 вынес жизненный цикл смены в шапку, собрал вкладку «Смена» (кокпит из `shiftRevenue`), слил payments+shifts, удалил старые воркспейсы. **Осталось: S2** (приходно-расходный ордер + лента/методы в «Журнал кассы» + X/Z read-only; ДОЛГ: вычистить осиротевшие `op.payments.*`) **→ S3** (слияние Продажи+Заказы). Детали/durable — в phase0-памяти.
...
- [Operator redesign — Этапы](operator-redesign-phase0-decisions.md) — Этапы 0/1/3/4 ЗАКРЫТЫ, 2 Касса в работе (S0 PR #109 + S1 смена PR #110 в main; впереди S2/S3), затем 5 Отчёты/6 Управление; currency = const-мапа `@afk4/money` (TJS→«с.», целые без дробной); App.test отдельным `bun test`-прогоном; флак-урок: дренировать вторую волну рефетчей после nonce-бампа.
