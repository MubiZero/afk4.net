...
- [Operator «Касса» (Этап 2)](operator-redesign-phase0-decisions.md) — **В РАБОТЕ**: **S0 (PR #109) + S1 смена (PR #110) + S2 достройка дыр (PR #111) в main**. Канон: spec `docs/superpowers/specs/2026-06-24-operator-cash-design.md`. Раздел = воркспейс `cash` (шапка-якорь `CashShiftHeader` + под-вкладки `sales`/`orders`/`shift`/`journal`). S1 — жизненный цикл смены в шапку + вкладка «Смена». S2 — вкладка «Журнал кассы» (`CashJournalWorkspace`: сегменты Кассовые операции `CashOperationsLedger` + Проверка `ReviewWorkspace embedded`), X-отчёт (снимок revenue) + Z-сводка (после close, печать), чистка −98 `op.payments.*` + `op.shell.nav.review`. **Осталось: ТОЛЬКО S3** (слияние Продажи+Заказы — самый рискованный, последний). Детали/durable — в phase0-памяти.
...
- [Operator redesign — Этапы](operator-redesign-phase0-decisions.md) — Этапы 0/1/3/4 ЗАКРЫТЫ, 2 Касса в работе (S0 PR #109 + S1 PR #110 + S2 PR #111 в main; впереди ТОЛЬКО S3), затем 5 Отчёты/6 Управление; currency = const-мапа `@afk4/money` (TJS→«с.», целые; разделитель тысяч = NBSP, отрицат. ASCII-минус); App.test отдельным `bun test`-прогоном; флак-урок: дренировать вторую волну рефетчей после nonce-бампа.
