...
- [Аудит Оператора 2026-08](operator-audit-backlog-2026-08.md) — **пункты 1-4 ЗАКРЫТЫ** (#150 баг POS + окно обновления, #151 мёртвый код и 816 ключей, #152 отключённые тесты). плюс #153 (гонка старта сессии → 500, многолетний флак CI). **ТОЧКА ПРОДОЛЖЕНИЯ в конце файла** — следующий кусок пользователь ещё не выбрал (мобильная обёртка / выгрузка журнала / мелкие долги). Durable: динамические ключи `roles.${}` наивный поиск не видит; `tsc` страхует удаление ключей.
...
- [Оператор = единая админка (эпик)](operator-as-unified-admin-epic.md) — стратегия: оператор абсорбирует owner-панель, доступ по роли, мобильная обёртка. **2 из 3 кусков ЗАКРЫТЫ** (проверено 2026-08-10): роли были готовы; **абсорбция owner-панели СДЕЛАНА** — раздел «Сеть» (`network/`: свод филиалов, подписка read-only, установка, журнал аудита) + сотрудники в «Управлении», отдельного owner-фронта нет; первый кирпич Клуб+Платежи в main. **Осталась только мобильная обёртка** (хоста нет, адаптив в 5 CSS из ~25). device-approval отменён.
