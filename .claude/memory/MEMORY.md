...
- [Operator «Касса» (Этап 2)](operator-redesign-phase0-decisions.md) — **В РАБОТЕ**: дизайн утверждён, **S0 (каркас) в PR #109** (ветка `feat/operator-cash`). Канон: spec `docs/superpowers/specs/2026-06-24-operator-cash-design.md` + план `docs/superpowers/plans/2026-06-24-operator-cash-s0-shell.md`. Решение: единый раздел = воркспейс `cash` (шапка-якорь смены `CashShiftHeader` + под-вкладки), смена = контекст. S0 переносит 5 воркспейсов (pos/shop_orders/payments/review/shifts) 1:1 без слияния; per-tab gating через `visibleCashTabs`. Слайсы: S0 каркас → S1 вкладка «Смена» (open/close/внести-изъять из шапки) → S2 дыры (приходно-расходный ордер, X/Z) → S3 слияние Продажи+Заказы. Бэк почти готов (дыры закрываются фронтом). Стартовые факты разведки — в phase0-памяти.
