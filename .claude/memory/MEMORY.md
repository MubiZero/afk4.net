...
> ## ⚠️ ПРОЕКТ ПЕРЕЕЗЖАЕТ НА FLUTTER (решено 2026-08-11)
> Весь UI переписывается с React/TypeScript+WPF на Flutter; стек = **C# + Dart**.
> Читать [Решение о переезде](flutter-migration-decision.md) ПЕРВЫМ — оно меняет статус почти
> всего ниже. **Новых функций нет**, бэклог заморожен; адаптив Оператора и мобильная обёртка
> ОТМЕНЕНЫ. Записи ниже описывают уходящую React-реализацию: они остаются ценны как источник
> поведения для переписывания, но больше не являются планом работ.
...
> Непомерженной работы в репозитории больше нет: 29.08.2026 всё лишнее вычищено, осталась одна
> ветка `main`. Осиротевшие ветки Оператора — [SecondaryApproval](operator-secondary-approval-orphan-branch.md)
> и [панель чеков](operator-pos-receipts-panel.md) — исчезли вместе с коммитами: идеи в этих файлах
> живы, кода за ними больше нет.

## Поведение / процесс
- [Переезд на Flutter](flutter-migration-decision.md) — **действующее решение**: весь UI на Dart, C#+Flutter, TypeScript и WPF уходят; порядок переезда, принятые развилки, заморозка бэклога. Карта функциональности — `docs/superpowers/specs/2026-08-11-flutter-migration-map.md`.
...
- [Волна C — наблюдение и аналитика](platform-observability-wave-c.md) — **ЗАКРЫТА ЦЕЛИКОМ**: здоровье/инциденты/оповещения (#140) + бизнес-аналитика (#141) + история клуба (#142) в main; связь клуба трёхзначна (null≠false), сутки по поясу филиала; дефекты всех трёх были в планах, не у исполнителей.
- [Волна D — продуктовые операции](platform-product-operations-wave-d.md) — **ЗАКРЫТА ЦЕЛИКОМ**: лимиты (#143), фичи (#144), роли (#145+#146, ревью #147), анонсы (#148), офбординг (#149). Durable: лестница «исключение→тариф→умолчание», письмо анонса ровно один раз, аудит режется по актору, `ExecuteDelete` нет на InMemory.
- [Волна B — цикл неплатежа и гибкая цена](platform-billing-dunning-wave-b.md) — **ЗАКРЫТА ЦЕЛИКОМ**: бэкенд #137 + доплата со скидкой #138 (лестница писем, авто-`past_due`, скидки, кредит-ноты, TJS), интерфейс #139 (раздел «Задолженность», блок долга в паспорте, полоса у клуба). Durable: «неизвестно» вместо «долгов нет» при 403; ICU-плюралы с первой строки.
...
- [Аудит Оператора 2026-08](operator-audit-backlog-2026-08.md) — **ЗАКРЫТ** (мобильная обёртка ОТМЕНЕНА переездом) (#150-#155). Новая находка: отчёт «Действия оператора» недостижим из UI, хотя жив на бэке и уходит в плановые рассылки. Durable: терминологический долг сведён наполовину — досводить по ВСЕМ языкам; функция теряется в коммите, отрезавшем навигацию, а не в коммите удаления; тесты Оператора гонять `bun run test`.
- [SecondaryApproval — потерянная ветка](operator-secondary-approval-orphan-branch.md) — подтверждение денежного действия вторым сотрудником на месте; ветка `feat/operator-reports-workspace-consolidation` и её коммиты исчезли из репозитория (проверено 2026-08-29). В main живёт очередь одобрений. Развилка «очередь против подтверждения на месте» не решена; понадобится — писать заново.
- [Статусы клиентов (эпик, бэклог)](operator-client-statuses-epic.md) — **ЗАМОРОЖЕН** переездом. Хотелка: система кастомных статусов с преференциями (скидки/бонусы/минуты), ручное+авто присвоение, применяется в Кассе/сессиях. Делается ПОСЛЕ редизайна Клиентов; база почти нулевая (только org-wide кэшбэк). НЕ начат.
...
- [Оператор = единая админка (эпик)](operator-as-unified-admin-epic.md) — **ЗАКРЫТ**: роли были готовы, **абсорбция owner-панели СДЕЛАНА** (раздел «Сеть»: свод филиалов, подписка read-only, установка, журнал аудита + сотрудники в «Управлении»; отдельного owner-фронта нет), третий кусок — мобильная обёртка — **ОТМЕНЁН переездом на Flutter**. device-approval отменён.
