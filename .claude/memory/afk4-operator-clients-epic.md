...
Этап 3 «Клиенты» программы редизайна Operator (см. [[operator-redesign-phase0-decisions]]). **S0 смержен в main** (merge-commit `82a55199`, PR #103: типизация API-клиента, feature-папка `src/players/`+тесты, dev-mock; no-behavior-change). **S1 готов** на ветке `feat/operator-clients-s1` (от свежего main) — 12 коммитов, выполнен через SDD (план `docs/superpowers/plans/2026-06-22-operator-clients-s1-redesign.md`, 11 задач). Сьют 302 pass/55 файлов + App.test 88/88 + i18n 34/34 + build чист; финальное whole-branch ревью (opus) = Ready to merge YES, 0 Critical/Important. Дальше по эпику: **S1b** (ledger keyset-эндпоинт + фильтр + «показать ещё»; история переключает источник с `recentEntries` на paged) → S2 (power-tools корректировка+возврат, PIN) → S3 (правка профиля + деактивация, новый бэкенд PATCH). Отложенные миноры S1 (некритичные, для S1b/полиша): двойная фильтрация поиска (сервер+клиент, унаследовано из монолита — убрать когда придёт серверная пагинация); нет теста на packageFallback; статус-бейдж строки `aria-hidden`. **Вне скоупа (отдельная POS-задача, #39):** React дубль-key варнинг `77777777…` в App.test — POS-фикстура (productId), не players. Раздел = воркспейс `players` (`BackendPlayersWorkspace.tsx`, ~650 строк, **НЕ заглушка**: поиск/профиль/кошелёк/пакеты/долг/создание уже работают). Сущность на бэке = **Player/PlayerAccount**, в вебе зовётся client.
